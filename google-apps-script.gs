/**
 * BO DIGITAL GSP — Google Sheets API v6.1.0
 *
...
const HEADERS = Object.freeze({
  ...
  BO_Acessos: ['UsuarioVigilante','RegistroVigilante','TurnoVigilante','LoginEm'],
...
function logAccessToSheet_(sheet, operator) {
  ...
  if (!user || !registration || !shift) return;

  if (sheet.getLastRow() > 1) {
    const rows = sheet.getRange(2,1,sheet.getLastRow()-1,4).getDisplayValues();
    const exists = rows.some(row => row[0] === user && row[1] === registration && row[2] === shift && row[3] === loginAt);
    if (exists) return;
  }

  sheet.appendRow([user,registration,shift,loginAt || new Date().toISOString()]);
}
