...
VERSÃO 17 — LOGIN DO VIGILANTE
- Usuário, registro e turno identificam o vigilante que usa o aplicativo.
- Os dados do solicitante são preenchidos separadamente em cada BO e não ficam salvos no aparelho.
- A planilha recebe os dados do vigilante em cada ocorrência e também registra os acessos na aba BO_Acessos.
- Substitua também o código do Apps Script pelo arquivo google-apps-script.gs desta versão e publique uma nova versão da implantação.

ATUALIZAÇÃO V18
- O login do vigilante permanece salvo no aparelho até usar “Encerrar turno”.
- A tela e o boletim em preenchimento são restaurados após fechar ou sair sem querer.
- O registro de login possui ID persistente e é reenviado à planilha quando a conexão voltar.
- O login também é confirmado ao sincronizar um BO, evitando perda do registro de acesso.
- Durante a finalização aparece “Aguarde o registro” até o salvamento e a sincronização terminarem.
- O Apps Script inclui a função testarRegistroLogin para validar manualmente a aba BO_Acessos.
