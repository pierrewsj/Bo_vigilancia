...
function materialSectionDescription(record = state.current) {
  ...
  if (policy.compactContext) return 'Cadastre o item encontrado ou relacionado ao fato. O formulário pergunta explicitamente se quantidade e identificação técnica puderam ser determinadas.';
  if (policy.loadContext) return 'Cadastre a carga/material e os dados disponíveis. Documentos da operação são registrados em um bloco próprio para evitar duplicidade.';
  return 'Cadastre somente os dados do material que estejam disponíveis e diretamente ligados ao fato.';
}
...
function assistantChecklist(record = state.current) {
  ...
  add('bo-origin','Origem do registro',meaningfulText(b.origemOcorrencia),'required',0,'Informe como o fato foi identificado.');
  ...
  if (requesterRequired(r)) {
    ...
    add('','Vigilante responsável pela ronda',meaningfulText(r.operator?.usuario) && meaningfulText(r.operator?.registro),'required',0,'A ronda deve ficar vinculada ao vigilante logado.','operator');
  }
  ...
  if (!isRoundOrigin(r)) add('history-start','Relato recebido',meaningfulText(h.inicio),'required',2,'Registre o que foi informado à equipe.');
...
function assistantMaterialSummary(material) {
  ...
  if (material.quantidadePrevista) details.push(`quantidade prevista ${material.quantidadePrevista}${material.unidade ? ` ${material.unidade}` : ''}`);
  if (material.quantidade) details.push(`${material.quantidadePrevista?'quantidade constatada':'quantidade'} ${material.quantidade}${material.unidade ? ` ${material.unidade}` : ''}`);
  ...
  if (material.quantidadePrevista && material.quantidade) {
    ...
    if (Number.isFinite(expected)&&Number.isFinite(found)) details.push(`divergência ${found-expected>0?'+':''}${found-expected}${material.unidade ? ` ${material.unidade}` : ''}`);
  }
...
function professionalTextReview(value = '') {
  ...
  if (!source) return '';
  const paragraphs = source.split(/\n{2,}/).map(reviewProfessionalParagraph).filter(Boolean);
  const unique = [];
  paragraphs.forEach(paragraph => {
    if (!unique.some(existing => similarReportText(existing, paragraph))) unique.push(paragraph);
  });
  return unique.join('\n\n');
}
...
function buildNarrative(record = state.current) {
  ...
  if (round) {
    ...
    introduction = `Em ${formatDateOnly(b.data)}, às ${b.hora}, durante ${lowerInitialPt(b.origemOcorrencia || 'ronda programada')}, o vigilante ${vigilante}${reg} registrou ocorrência${local ? ` no local ${local}` : ''}`;
    if (reference) introduction += ` classificada como ${lowerInitialPt(reference)}`;
    introduction += '.';
  } else {
    introduction = `Em ${formatDateOnly(b.data)}, às ${b.hora}, a equipe de Segurança Patrimonial iniciou atendimento${local ? ` no local ${local}` : ''}`;
    if (reference) introduction += ` para registro de ${lowerInitialPt(reference)}`;
    introduction += '.';
  }
  paragraphs.push(introduction);

  if (submodel?.label) paragraphs.push(`O registro foi conduzido conforme o roteiro operacional “${submodel.label}”.`);
  if (!round && meaningfulText(b.nomeEmissor)) {
    paragraphs.push(`O atendimento foi solicitado por ${titleCaseNamePt(b.nomeEmissor)}${b.matriculaEmissor ? `, matrícula ${b.matriculaEmissor}` : ''}.`);
  }
  if (directorate && !['Não identificada','Não se aplica'].includes(directorate)) {
    paragraphs.push(`A ocorrência foi vinculada à diretoria ${directorate}${b.setorArea ? `, setor/área ${b.setorArea}` : ''}.`);
  } else if (b.setorArea) {
    paragraphs.push(`A ocorrência foi relacionada ao setor/área ${b.setorArea}, sem diretoria confirmada no momento do registro.`);
  }
  ...
  if (!round) {
    ...
    if (received) {
      const lead = reportSourceLead(record, rawReceived);
      paragraphs.push(`${lead}, ${lowerInitialPt(received)}.`);
    }
  }

  const found = stripFindingLead(h.identificado);
  if (found) paragraphs.push(`${round ? 'Durante a ronda' : 'No local'}, a equipe de Segurança Patrimonial registrou a seguinte constatação: ${sentenceCasePt(found)}.`);

  const structuredFacts = templateNarrativeEntries(record).filter(item => item.kind === 'fact');
  if (structuredFacts.length) {
    const chunks = [];
    for (let i = 0; i < structuredFacts.length; i += 4) chunks.push(structuredFacts.slice(i, i + 4));
    chunks.forEach((chunk, index) => {
      const details = chunk.map(item => `${item.question.label.replace(/[?:]$/,'')} — ${item.value}`).join('; ');
      paragraphs.push(`${index === 0 ? 'Na apuração, foram registrados os seguintes dados complementares' : 'Também foram registrados'}: ${details}.`);
    });
  }

  if (requesterCountsAsInvolved(record)) paragraphs.push('O solicitante também foi identificado como pessoa diretamente envolvida na ocorrência.');
  ...
  if ((record.documents || []).length) paragraphs.push(`Documentos relacionados: ${joinNatural(record.documents.map(assistantDocumentSummary))}.`);
  if (meaningfulText(record.templateData?.evidenceUnavailableReason)) {
    paragraphs.push(`Não foi possível registrar toda a evidência prevista. Motivo informado: ${cleanSentence(record.templateData.evidenceUnavailableReason)}.`);
  }

  const providences = meaningfulText(h.providencias) || buildAssistantProvidences(record);
  if (providences) paragraphs.push(providences);

  const outcome = cleanSentence(h.desfecho);
  if (outcome) {
    const cleanedOutcome = outcome.replace(/^ao\s+t[eé]rmino\s+do\s+atendimento\s*[,;:\-–—]*\s*/i, '').trim();
    paragraphs.push(`Ao término do atendimento, ${lowerInitialPt(cleanedOutcome)}.`);
  }
  const additional = cleanSentence(h.adicional);
  if (additional) paragraphs.push(`Como informação complementar, ${lowerInitialPt(additional)}.`);
  return professionalTextReview(paragraphs.join('\n\n'));
}
...
function normalizeRecord(record) {
  ...
  normalized.schemaVersion = Math.max(7, Number(normalized.schemaVersion || 4));
  normalized.basic ||= {};
...
function renderSubmodelField(record = state.current) {
  const items = submodelsFor(record?.basic?.referencia);
  if (!items.length) return '';
  const selected = String(record?.templateData?.submodel || '');
  const options = `<option value="">Situação geral deste tipo</option>${items.map(item=>`<option value="${escapeHtml(item.id)}" ${item.id===selected?'selected':''}>${escapeHtml(item.label)}</option>`).join('')}`;
  return `<div class="field full submodel-selector"><label for="bo-submodel">Situação específica <span class="question-level recommended">quando corresponder</span></label><select id="bo-submodel" data-path="templateData.submodel" data-rerender="true">${options}</select><small>Escolha uma especialização quando ela representar exatamente o cenário. Isso ajusta perguntas, documentos, materiais e evidências.</small>${selected?`<div class="selected-submodel-note">${escapeHtml(items.find(item=>item.id===selected)?.label||selected)}</div>`:''}</div>`;
}

function submodelSelectLabelFix(root = document) {
  const select = root.querySelector('#bo-submodel');
  if (!select) return;
  const items = submodelsFor(state.current?.basic?.referencia);
  Array.from(select.options).forEach(option=>{ const item=items.find(x=>x.id===option.value); if(item) option.textContent=item.label; });
  syncModalSelectButton(select);
}
...
function renderRoutingFields(record = state.current) {
  ...
  const controls = questions.map(question=>{
    if (locked[question.id] !== undefined) {
      return `<div class="field full template-question required"><div class="template-question-label"><label>${escapeHtml(question.label)}</label><span class="question-level required">Definido pelo roteiro</span></div><div class="origin-fixed-value"><strong>${escapeHtml(locked[question.id])}</strong><span>Preenchido automaticamente pela situação específica selecionada.</span></div></div>`;
    }
...
function basicOriginFields(record = state.current) {
  ...
  const value = forcedRound || b.origemOcorrencia || defaultOriginForReference(reference);
  if (forcedRound) {
    return `<div class="field full origin-fixed-card"><label>Origem do registro</label><div class="origin-fixed-value"><strong>${escapeHtml(forcedRound)}</strong><span>Ronda operacional programada da Segurança Patrimonial.</span></div></div>`;
  }
  return `<div class="field full"><label class="required" for="bo-origin">Como o fato foi identificado?</label><select id="bo-origin" data-path="basic.origemOcorrencia" data-rerender="true" required>${selectOptions(ORIGIN_OPTIONS,value)}</select><small>Essa informação adapta a identificação do solicitante e a redação do BO. Se o fato foi identificado em ronda, selecione a ronda correspondente.</small><span class="field-error" data-error-for="bo-origin"></span></div>`;
}
...
function renderBasicStep() {
  ...
  return `
    ...
    <section class="form-card">
      ...
      <p>Classifique a ocorrência primeiro. As respostas de direcionamento abaixo ajustam automaticamente pessoas, veículos, materiais, documentos e evidências.</p>
      <div class="form-grid">
        ${basicReferenceFields(b)}
        ${renderSubmodelField(state.current)}
    ...
    ${round ? roundResponsibleCard(state.current) : `<section class="form-card compact-card"><p class="eyebrow">Solicitante</p><h2>Quem acionou a Segurança?</h2><div class="form-grid">${basicRequesterFields(b)}</div></section>`}
    <section class="form-card compact-card"><p class="eyebrow">Área relacionada</p><h2>Diretoria e setor</h2><div class="form-grid">${basicDirectorateFields(b)}</div></section>
    <div class="notice info"><strong>Fluxo adaptativo:</strong> itens opcionais deixam de exigir confirmações negativas. O aplicativo cobrará apenas dados essenciais ao cenário selecionado.</div>`;
}
...
function renderRelatedStep() {
  ...
  if (modes.materials === 'required') blocks.push(requiredEntitySection('materials',{key:'materials',title:'Material, peça, equipamento ou carga',description:materialSectionDescription(state.current),button:'Adicionar material',empty:'Nenhum material cadastrado.',render:renderMaterialItem}));
...
function renderTemplateInsight(record = state.current) {
  ...
  if (reference === 'Divergência de carga, quantidade ou documentação' && policy.expectedQuantityCheck) {
    const rows = (record.materials||[]).filter(material=>material.quantidadePrevista && material.quantidade).map(material=>{
      ...
      return {name:material.denominacao||'Material',expected,observed,difference,unit:material.unidade||''};
    }).filter(Boolean);
    if (rows.length) {
      return `<div class="template-calculated-insight ${rows.some(row=>row.difference!==0)?'attention':'ok'}"><span>${rows.some(row=>row.difference!==0)?ICONS.warning:ICONS.check}</span><div><strong>Conferência automática de quantidade</strong><div class="quantity-comparison-list">${rows.map(row=>`<p><b>${escapeHtml(row.name)}</b>: prevista ${escapeHtml(row.expected)}${row.unit?` ${escapeHtml(row.unit)}`:''} • constatada ${escapeHtml(row.observed)}${row.unit?` ${escapeHtml(row.unit)}`:''} • diferença ${row.difference>0?'+':''}${escapeHtml(row.difference)}${row.unit?` ${escapeHtml(row.unit)}`:''}</p>`).join('')}</div></div></div>`;
    }
  }
  return '';
}
...
function renderInvestigationStep() {
  ...
  const sourceBlock = round
    ? `<section class="form-card fact-source-card round-finding-card"><div class="field full"><div class="fact-source-title"><span>1</span><div><strong>Constatação da ronda</strong><small>Registre somente fatos observados ou verificados pelo vigilante.</small></div></div><label class="required" for="history-found">O que foi constatado durante a ronda?</label><textarea id="history-found" data-path="history.identificado" placeholder="Descreva objetivamente a condição encontrada durante a ronda." required>${escapeHtml(h.identificado)}</textarea><span class="field-error" data-error-for="history-found"></span></div></section>`
    : `<section class="form-card fact-source-card"><div class="fact-source-grid"><div class="field full"><div class="fact-source-title"><span>1</span><div><strong>Relato recebido</strong><small>Informação de solicitante, envolvido ou terceiro</small></div></div><label class="required" for="history-source">Fonte principal do relato</label><select id="history-source" data-path="history.fonteRelato" required>${selectOptions(['Solicitante','Pessoa envolvida','Testemunha','Terceiro/outro informante','Fonte não identificada'],h.fonteRelato||'Solicitante')}</select><label class="required" for="history-start">O que foi informado à equipe?</label><textarea id="history-start" data-path="history.inicio" placeholder="Escreva apenas o conteúdo informado; o sistema adicionará a fonte do relato." required>${escapeHtml(h.inicio)}</textarea><span class="field-error" data-error-for="history-start"></span></div><div class="field full"><div class="fact-source-title"><span>2</span><div><strong>Constatação da Segurança</strong><small>Somente fatos observados ou verificados</small></div></div><label class="required" for="history-found">O que a equipe constatou no local?</label><textarea id="history-found" data-path="history.identificado" placeholder="Registre somente aquilo que foi efetivamente observado ou confirmado." required>${escapeHtml(h.identificado)}</textarea><span class="field-error" data-error-for="history-found"></span></div></div></section>`;
  return `<section class="form-card step-intro-card"><p class="eyebrow">3. Apuração</p><h1>Apuração guiada: ${escapeHtml(resolvedReference(r.basic))}</h1><p>${round?'A ronda não possui solicitante. Registre a constatação do vigilante e responda apenas às perguntas específicas do fato.':'Separe o que foi informado por terceiros daquilo que a Segurança efetivamente constatou.'}</p></section>${sourceBlock}
  <section class="form-card template-question-card"><div class="template-question-head"><div><p class="eyebrow">Roteiro padronizado</p><h2>Informações específicas que não podem ser esquecidas</h2><p>${escapeHtml(template.guidance || '')}</p></div><div class="quality-mini ${completeness.complete?'complete':'pending'}"><strong>${completeness.required.length-completeness.requiredMissing.length}/${completeness.required.length}</strong><span>essenciais</span></div></div><div class="form-grid template-form-grid">${questions.map(q=>renderTemplateQuestion(q,r)).join('')}</div>${renderTemplateInsight(r)}${!questions.length?'<div class="entity-empty">Nenhuma pergunta adicional é necessária. Continue para providências e evidências.</div>':''}</section>
...
function renderEvidenceBlock() {
  ...
  const requirementCards = requirements.length ? `<div class="evidence-requirement-list">${requirements.map(item=>`<article class="evidence-requirement ${item.satisfied?'done':item.required?'pending':''}"><span class="evidence-requirement-icon">${item.satisfied?ICONS.check:ICONS.file}</span><div><strong>${escapeHtml(item.label)}</strong><small>${item.required?'Essencial':'Recomendado'}${item.safety?' • somente se houver condição segura':''}</small></div>${item.satisfied?'<span class="evidence-ok">Registrada</span>':`<button class="button ${item.required?'primary':'secondary'} small" type="button" data-evidence-purpose="${escapeHtml(item.id)}">${ICONS.plus} ${item.type==='photo'?'Tirar foto':'Adicionar'}</button>`}</article>`).join('')}</div>` : '';
...
function renderMaterialItem(material, index) {
  const quantity = material.quantidade
    ? `<span class="chip">${escapeHtml(material.quantidade)} ${escapeHtml(material.unidade||'')}</span>`
    : material.quantidadeStatus === 'Não foi possível determinar' ? '<span class="chip">Quantidade não determinada</span>' : '';
  return `<article class="entity-item">
    <div><h3>${escapeHtml(material.denominacao || 'Material sem denominação')}</h3><div class="entity-meta">${quantity}${material.desenho ? `<span class="chip">Cód. ${escapeHtml(material.desenho)}</span>` : ''}${material.pessoaNome ? `<span class="chip">Ligado a ${escapeHtml(material.pessoaNome)}</span>` : ''}</div></div>
...
function reviewItems(record = state.current) {
  ...
  return [
    make('occurrence','Identificação da ocorrência',0,step0,'bo-ref',isRoundOrigin(record)?'Classificação, origem, local, vigilante responsável e área conferidos.':'Classificação, origem, local, solicitante e área conferidos.'),
    make('essentials','Dados essenciais',1,step1,'','Pessoas, veículos, materiais e documentos exigidos pelo cenário foram conferidos.'),
    make('investigation','Apuração padronizada',2,step2,isRoundOrigin(record)?'history-found':'history-start',isRoundOrigin(record)?'Constatação da ronda e roteiro específico concluídos.':'Relato recebido, constatação e roteiro específico concluídos.'),
...
function renderReviewStep() {
  ...
  const identityBlock = round
    ? `<div><dt>Origem</dt><dd>${escapeHtml(b.origemOcorrencia||'Ronda')}</dd></div><div><dt>Vigilante da ronda</dt><dd>${escapeHtml(r.operator?.usuario||'Não identificado')} • ${escapeHtml(r.operator?.registro||'')}</dd></div>`
  ...
  const investigationSource = round
    ? `<div style="grid-column:1/-1"><dt>Constatação da ronda</dt><dd>${escapeHtml(r.history.identificado||'Não informado')}</dd></div>`
  ...
  return `<div class="review-professional-layout v32-review">
    ...
    <div class="review-detail-panel">
      <section class="review-context-card"><div><p class="eyebrow">${escapeHtml(b.categoria || referenceCategory(b.referencia))}</p><h2>${escapeHtml(reference||'Ocorrência não classificada')}</h2>${selectedSubmodel(r)?`<p><strong>Roteiro:</strong> ${escapeHtml(selectedSubmodel(r).label)}</p>`:''}<p>Revise o conteúdo abaixo. O término do atendimento será registrado automaticamente ao finalizar.</p></div><span class="review-number">${escapeHtml(isOfficialNumber(r.numero)?r.numero:'Número gerado ao enviar')}</span></section>
      ...
      <div class="review-grid compact-review-grid">
        ...
        ${reviewSection('Relato consolidado',4,`<div class="final-report-preview">${escapeHtml(r.history.relato||'Relato ainda não gerado.').replace(/\n/g,'<br>')}</div><div class="about-actions"><button class="button secondary small" type="button" data-action="assistant-generate">${ICONS.sync} Atualizar relato</button><button class="button secondary small" type="button" data-action="assistant-review-text">${ICONS.check} Revisar redação</button></div>`, 'report')}
      </div>
...
function renderBasicCorrection(focus) {
  ...
  if (['bo-date','bo-time','bo-ref','bo-ref-other','bo-origin'].includes(id)) {
    ...
    fields = `<div class="field"><label class="required" for="bo-date">Data da ocorrência</label><input id="bo-date" type="date" value="${escapeHtml(b.data)}" data-path="basic.data" required></div><div class="field"><label class="required" for="bo-time">Hora da ocorrência</label><input id="bo-time" type="time" value="${escapeHtml(b.hora)}" data-path="basic.hora" required></div>${basicReferenceFields(b)}${renderSubmodelField(state.current)}${renderRoutingFields(state.current)}${basicOriginFields(state.current)}${round?'':basicRequesterFields(b)}${basicLocationFields(b)}${basicDirectorateFields(b)}`;
  }
...
function renderReportCorrection() {
  const h = state.current.history;
  return `${renderAssistantCard(state.current)}<section class="form-card correction-card report-manual-editor" data-correction-section="report"><p class="eyebrow">Correção manual do relatório</p><h1>Editar relato consolidado</h1><p>Faça diretamente os ajustes de concordância, pontuação, clareza ou organização que considerar necessários. O NEXO auxilia na revisão automática, mas a decisão final de redação permanece com o vigilante.</p><div class="form-grid">
...
function renderFocusedCorrection(focus) {
  ...
  const requiredMaterials = () => requiredEntitySection('materials',{key:'materials',title:'Material, peça, equipamento ou carga',description:materialSectionDescription(state.current),button:'Adicionar material',empty:'Nenhum material cadastrado.',render:renderMaterialItem});
  const configs = {
...
function renderMaterialItemReview(m) { const qty=m.quantidade?`<span class="chip">${escapeHtml(m.quantidade)} ${escapeHtml(m.unidade||'')}</span>`:(m.quantidadeStatus==='Não foi possível determinar'?'<span class="chip">Quantidade não determinada</span>':''); return `<div><strong>${escapeHtml(m.denominacao||'Item sem identificação técnica')}</strong><div class="entity-meta">${qty}</div></div>`; }
  ...
  app.querySelectorAll('[data-path]').forEach(control => {
    const update = () => {
      ...
      if (control.dataset.path === 'templateData.submodel') {
        ...
        if (sub?.origin) state.current.basic.origemOcorrencia = sub.origin;
...
function bindStepSpecific() {
  ...
  app.querySelectorAll('[data-select-reference]').forEach(button => button.addEventListener('click', async()=>{
    ...
    if (previous !== reference) {
      ...
      state.current.templateData.evidenceUnavailableReason = '';
      state.current.templateData.documentUnavailableReason = '';
      state.current.basic.origemOcorrencia = defaultOriginForReference(reference);
      ['people','witnesses','vehicles','materials'].forEach(key => { state.current.verification[key] = 'pending'; });
    }
...
function collectStepIssues(step, record = state.current) {
  ...
  if (step === 0) {
    ...
    if (!String(b.origemOcorrencia||'').trim()) add('bo-origin','Informe como o fato foi identificado.');
    ...
    if (requesterRequired(record)) {
      ...
      if (!String(record.operator?.usuario||'').trim()) add('','A ronda deve ficar vinculada ao nome do vigilante logado.','Vigilante da ronda não identificado','operator');
      if (!String(record.operator?.registro||'').trim()) add('','A ronda deve ficar vinculada ao registro do vigilante logado.','Registro do vigilante ausente','operator');
    }
  ...
  if (step === 1) {
    ...
    (record.materials||[]).forEach((material,index)=>{
      if (!String(material.denominacao||'').trim()) add('','Informe a denominação/descrição do material.','Material incompleto','materials');
      if (policy.quantityCheck) {
        if (!String(material.quantidadeStatus||'').trim()) add('','Confirme se a quantidade do material pôde ser determinada.','Quantidade não verificada','materials');
        if (material.quantidadeStatus === 'Determinada' && (!String(material.quantidade||'').trim() || !String(material.unidade||'').trim())) add('','Quando a quantidade é determinada, informe quantidade e unidade.','Quantidade incompleta','materials');
      ...
      if (policy.technicalIdCheck && material.identificacaoTecnicaStatus === 'Disponível' && !String(material.desenho||'').trim()) add('','Informe o desenho/código da peça quando a identificação técnica foi marcada como disponível.','Identificação técnica incompleta','materials');
    });
  ...
  if (step === 2) {
    const h = record.history || {};
    if (!isRoundOrigin(record) && !String(h.inicio||'').trim()) add('history-start','Registre o que foi informado à equipe.');
...
function personForm(p) {
  ...
  return `<div class="entity-context-note"><strong>Identificação possível</strong><span>Registre somente os dados que puder confirmar. Matrícula, documento e empresa não devem ser inventados para concluir o cadastro.</span></div><div class="form-grid">
...
function materialForm(m) {
  ...
  const technicalStatus = m.identificacaoTecnicaStatus || (m.desenho ? 'Disponível' : '');
  const quantityRequired = policy.quantityRequired && quantityStatus !== 'Não foi possível determinar';
  const complementaryFields = [
    policy.showSupplier ? `<div class="field"><label for="material-supplier">Fornecedor</label><input id="material-supplier" name="fornecedor" value="${escapeHtml(m.fornecedor||'')}" placeholder="Quando conhecido"></div>` : '',
    policy.showDrawing ? `<div class="field technical-id-field"><label class="${technicalStatus==='Disponível'?'required':''}" for="material-drawing">Desenho / código da peça</label><input id="material-drawing" name="desenho" value="${escapeHtml(m.desenho||'')}" placeholder="Informe quando houver identificação técnica disponível" ${technicalStatus==='Disponível'?'required':''}></div>` : '',
    policy.showContainer ? `<div class="field"><label for="material-container">Código do vasilhame/container</label><input id="material-container" name="codigoVasilhame" value="${escapeHtml(m.codigoVasilhame||'')}" placeholder="Quando aplicável"></div>` : ''
  ].filter(Boolean).join('');
  return `<div class="entity-context-note"><strong>Cadastro adaptado ao tipo de BO</strong><span>${escapeHtml(policy.guidance)}</span></div><div class="form-grid">
    <div class="field full"><label class="required" for="material-name">Descrição / denominação</label><input id="material-name" name="denominacao" value="${escapeHtml(m.denominacao||'')}" required placeholder="Nome ou descrição objetiva da peça/material"><button type="button" class="field-helper-button" data-material-unidentified>Item sem identificação técnica</button></div>
    ${policy.quantityCheck?`<div class="field"><label class="required" for="material-quantity-status">A quantidade pôde ser determinada?</label><select id="material-quantity-status" name="quantidadeStatus" required>${selectOptions(['Determinada','Não foi possível determinar'],quantityStatus)}</select></div>`:''}
    ${policy.expectedQuantityCheck?`<div class="field expected-quantity-field"><label class="required" for="material-expected-quantity">Quantidade prevista/documentada</label><input id="material-expected-quantity" type="number" min="0" step="0.01" name="quantidadePrevista" value="${escapeHtml(m.quantidadePrevista||'')}" required placeholder="Quantidade prevista no documento"></div>`:''}
    <div class="field quantity-value-field"><label class="${quantityRequired?'required':''}" for="material-quantity">${policy.expectedQuantityCheck?'Quantidade constatada':'Quantidade'}</label><input id="material-quantity" type="number" min="0.01" step="0.01" name="quantidade" value="${escapeHtml(m.quantidade||'')}" ${quantityRequired?'required':''} placeholder="${policy.expectedQuantityCheck?'Quantidade física constatada':'Quantidade constatada'}"></div>
    <div class="field quantity-value-field"><label class="${quantityRequired?'required':''}" for="material-unit">Unidade</label><input id="material-unit" name="unidade" value="${escapeHtml(m.unidade||'')}" placeholder="un, kg, cx..." ${quantityRequired?'required':''}></div>
    ${policy.quantityCheck?`<div class="field full quantity-reason-field"><label for="material-quantity-reason">Motivo da quantidade não determinada</label><textarea id="material-quantity-reason" name="quantidadeIndisponivelMotivo" placeholder="Explique por que não foi possível determinar a quantidade com segurança.">${escapeHtml(m.quantidadeIndisponivelMotivo||'')}</textarea></div>`:''}
    ${policy.technicalIdCheck?`<div class="field"><label class="required" for="material-tech-status">Foi possível obter identificação técnica?</label><select id="material-tech-status" name="identificacaoTecnicaStatus" required>${selectOptions(['Disponível','Não disponível / não aplicável'],technicalStatus)}</select></div>`:''}
    ${complementaryFields ? `<div class="field full material-complementary-wrap"><details class="material-complementary" ${policy.loadContext||technicalStatus==='Disponível'?'open':''}><summary>Identificação complementar <span>quando disponível</span></summary><div class="form-grid">${complementaryFields}</div></details></div>` : ''}
    ...
    <div class="field full"><label for="material-notes">Condição / observações</label><textarea id="material-notes" name="observacao" placeholder="Condição encontrada, características, local ou informação que ajude a identificar o item.">${escapeHtml(m.observacao||'')}</textarea></div>
  </div>`;
}
...
function bindDialogDynamic(type) {
  ...
  if(type==='materials'){
    ...
    const qty=dialogBody.querySelector('#material-quantity'),unit=dialogBody.querySelector('#material-unit'),status=dialogBody.querySelector('#material-quantity-status'),reason=dialogBody.querySelector('#material-quantity-reason'),tech=dialogBody.querySelector('#material-tech-status'),drawing=dialogBody.querySelector('#material-drawing');
    const update=()=>{
      ...
      if(unit)unit.required=!unavailable && (policy.quantityRequired || policy.quantityCheck || Boolean(String(qty?.value||'').trim()));
      if(reason)reason.required=Boolean(unavailable);
      if(drawing)drawing.required=tech?.value==='Disponível';
      refreshRequiredFieldStates(dialogBody);
    };
    qty?.addEventListener('input',update);status?.addEventListener('change',update);tech?.addEventListener('change',update);update();
    dialogBody.querySelector('[data-material-unidentified]')?.addEventListener('click',()=>{
      ...
      if(tech)tech.value='Não disponível / não aplicável';
      update();showToast('Item registrado sem identificação técnica.');
    });
  }
}
...
async function saveDialogEntity(event) {
  ...
  if(type==='materials'){
    const policy=materialFieldPolicy(state.current,index===null?{}:state.current.materials[index]||{});
    const unavailable=formData.quantidadeStatus==='Não foi possível determinar';
    if(unavailable){formData.quantidade='';formData.unidade='';if(!String(formData.quantidadeIndisponivelMotivo||'').trim()){await openAppModal({kind:'warning',eyebrow:'Quantidade não determinada',title:'Informe o motivo',message:'Explique por que não foi possível determinar a quantidade com segurança.',confirmText:'Completar cadastro'});return;}}
    ...
    if(hasQuantity&&!String(formData.unidade||'').trim()){await openAppModal({kind:'warning',eyebrow:'Unidade necessária',title:'Informe a unidade da quantidade',message:'Ex.: un, kg, cx.',confirmText:'Completar cadastro'});return;}
    if(policy.technicalIdCheck&&formData.identificacaoTecnicaStatus==='Disponível'&&!String(formData.desenho||'').trim()){await openAppModal({kind:'warning',eyebrow:'Identificação técnica',title:'Informe o desenho/código',message:'Foi indicado que a identificação técnica está disponível. Informe o desenho ou código correspondente.',confirmText:'Completar cadastro'});return;}
    formData.pessoaNome=personNameById(formData.pessoaId);
  }
  if(type==='documents'){
...
function recordMatches(record,rawTerm){
  ...
  const haystack=[record.numero,record.numeroTemporario,record.basic?.referencia,record.basic?.referenciaOutra,record.basic?.origemOcorrencia,record.basic?.diretoria,record.basic?.diretoriaOutra,record.basic?.nomeEmissor,record.basic?.local,record.basic?.complementoLocal,selectedSubmodel(record)?.label,...(record.people||[]).map(p=>`${p.nome} ${p.matricula} ${p.numeroDocumento}`),...(record.vehicles||[]).map(v=>`${v.placa} ${v.marca} ${v.modelo}`),...(record.materials||[]).map(m=>`${m.denominacao} ${m.desenho} ${m.fornecedor}`),...(record.documents||[]).map(d=>`${d.tipo} ${d.numero}`)].join(' ').toLowerCase();
...
function renderDetail() {
  ...
  <div class="review-grid">
    ${reviewSectionStatic('Ocorrência',`<dl class="definition-grid"><div><dt>Categoria</dt><dd>${escapeHtml(b.categoria||referenceCategory(b.referencia)||'Não informada')}</dd></div><div><dt>Referência</dt><dd>${escapeHtml(reference||'Não informada')}</dd></div>${selectedSubmodel(r)?`<div><dt>Roteiro</dt><dd>${escapeHtml(selectedSubmodel(r).label)}</dd></div>`:''}${identity}<div><dt>Local</dt><dd>${escapeHtml(b.local)} — ${escapeHtml(b.complementoLocal)}</dd></div><div><dt>Diretoria / área</dt><dd>${escapeHtml(directorate||'Não informada')}${b.setorArea?` • ${escapeHtml(b.setorArea)}`:''}</dd></div><div><dt>Data/hora do fato</dt><dd>${formatDateOnly(b.data)} às ${escapeHtml(b.hora)}</dd></div><div><dt>Início do BO</dt><dd>${formatDateTime(r.operationalTiming?.startedAt||r.createdAt)}</dd></div><div><dt>Finalização</dt><dd>${formatDateTime(r.finalizedAt)}</dd></div><div><dt>Duração do atendimento</dt><dd>${escapeHtml(attendance.label)}</dd></div></dl>`)}
