...
const PROFILE_KEY = 'bo-digital-emitter-profile-v1';
...
function saveEmitterProfile(profile) {
...
function createBlankRecord() {
  const now = new Date();
  const profile = state.profile || loadEmitterProfile() || {};
  ...
  return {
    ...
    saveEmitterProfile: Boolean(profile.nomeEmissor || profile.matriculaEmissor),
    basic: {
      data: localDateInput(now), hora: localTimeInput(now), referencia: '', referenciaOutra: '',
      matriculaEmissor: profile.matriculaEmissor || '', nomeEmissor: profile.nomeEmissor || '',
      emailEmissor: profile.emailEmissor || '', turnoEmissor: profile.turnoEmissor || '',
...
async function saveCurrent(silent = true, autoSync = true) {
  ...
  if (state.current.saveEmitterProfile && state.current.basic.nomeEmissor && state.current.basic.matriculaEmissor) {
    saveEmitterProfile({
      nomeEmissor: state.current.basic.nomeEmissor,
      matriculaEmissor: state.current.basic.matriculaEmissor,
      emailEmissor: state.current.basic.emailEmissor || '',
      turnoEmissor: state.current.basic.turnoEmissor || ''
    });
  }
...
function scheduleSave() {
  ...
  autosaveTimer = setTimeout(() => saveCurrent(true, false), 500);
}
...
function normalizeRecord(record) {
  ...
  normalized.history ||= {};
  normalized.history = {
    identificado: normalized.history.identificado || '', inicio: normalized.history.inicio || '',
    presentes: normalized.history.presentes || '', providencias: normalized.history.providencias || '',
    acionados: normalized.history.acionados || '', desfecho: normalized.history.desfecho || '',
    adicional: normalized.history.adicional || '', relato: normalized.history.relato || '',
    relatoEditado: Boolean(normalized.history.relatoEditado), observacoes: normalized.history.observacoes || ''
  };
  normalized.verification ||= {};
  normalized.verification = {
    people: normalized.verification.people || (normalized.people.length ? 'has' : normalized.peopleNone ? 'none' : 'pending'),
  ...
  normalized.saveEmitterProfile = Boolean(normalized.saveEmitterProfile);
  ...
  delete normalized.basic.subreferencia; delete normalized.basic.subreferenciaOutra; delete normalized.peopleNone;
...
async function syncRecord(record, notify = true) {
  ...
  try {
    const result = await apiPost({ action: 'upsert', record: recordForSync(record) });
    if (result.officialNumber && result.officialNumber !== record.numero) {
    ...
    await dbPut(record);
    if (state.current?.id === record.id) state.current = record;
    ...
    await dbPut(record);
    if (notify) await openAppModal({ kind: 'danger', eyebrow: 'Falha de sincronização', title: 'Não foi possível enviar o boletim', message: escapeHtml(error.message), details: 'O registro permanece salvo no aparelho. Toque na situação de sincronização para tentar novamente.', confirmText: 'Entendi' });
    return false;
  }
}
...
function renderHome() {
  ...
  app.innerHTML = `
    ...
    <div class="technical-link-wrap"><button class="technical-link" type="button" data-action="show-bank">${ICONS.gear} Configurações técnicas</button></div>
  `;
...
async function openRecord(id) {
  ...
  if(!record)return;
  const normalized=normalizeRecord(record);await dbPut(normalized);await refreshRecords();state.current=structuredClone(normalized);
  ...
  app.innerHTML = `
    <div class="step-shell">
      <section class="progress-card">
        ...
        <div class="record-sync-line"><span class="sync-state sync-${escapeHtml(state.current.syncStatus || 'local')}"><i></i>${escapeHtml(syncStatusLabel(state.current))}</span><span>${escapeHtml(state.current.numero)}</span></div>
      </section>
      ${content}
...
function renderBasicStep() {
  ...
  return `
    <section class="form-card">
      ...
      <p>Responda aos campos na ordem. As orientações mudam conforme a opção escolhida.</p>
      <div class="form-grid">
        ...
        <div class="field"><label class="required" for="bo-reg">Matrícula do emissor</label><input id="bo-reg" type="text" inputmode="numeric" pattern="[0-9]*" value="${escapeHtml(b.matriculaEmissor)}" data-path="basic.matriculaEmissor" placeholder="Ex.: 76313" required><span class="field-error" data-error-for="bo-reg"></span></div>
        <div class="field"><label class="required" for="bo-name">Nome do emissor</label><input id="bo-name" type="text" value="${escapeHtml(b.nomeEmissor)}" data-path="basic.nomeEmissor" autocomplete="name" placeholder="Nome completo" required><span class="field-error" data-error-for="bo-name"></span></div>
        <div class="field"><label for="bo-email">E-mail do emissor</label><input id="bo-email" type="email" value="${escapeHtml(b.emailEmissor)}" data-path="basic.emailEmissor" autocomplete="email" placeholder="nome@empresa.com"><span class="field-error" data-error-for="bo-email"></span></div>
        <div class="field"><label for="bo-shift">Turno</label><select id="bo-shift" data-path="basic.turnoEmissor">${selectOptions(['1º turno','2º turno','3º turno','Administrativo'], b.turnoEmissor, 'Selecione, se aplicável')}</select></div>
        <div class="field full">
          <label class="checkbox-card compact ${state.current.saveEmitterProfile ? 'checked' : ''}"><input type="checkbox" data-path="saveEmitterProfile" ${state.current.saveEmitterProfile ? 'checked' : ''}><span><strong>Salvar meus dados neste aparelho</strong><span>Nos próximos boletins, nome, matrícula, e-mail e turno serão preenchidos automaticamente.</span></span></label>
        </div>
...
function verificationChoice(key, label, hasItems, noneLabel) {
  ...
  return `<div class="verification-control" data-verification-key="${key}">
...
function relatedSection(type, config) {
  ...
  return `<section class="related-block ${config.recommended ? 'recommended' : ''}">
    ...
    ${status === 'pending' ? '<div class="notice warning compact-notice">Escolha “Vou cadastrar” ou confirme que não se aplica.</div>' : ''}
...
function buildNarrative(record = state.current) {
  ...
  const parts = [];
  parts.push(`Em ${formatDateOnly(b.data)}, às ${b.hora}, no local ${local || 'informado no boletim'}, foi registrada a ocorrência classificada como ${b.referencia === 'Danos materiais — Outra' ? b.referenciaOutra : b.referencia}.`);
  if (h.inicio) parts.push(h.inicio.trim().replace(/[.]+$/,'') + '.');
  if (h.identificado) parts.push(`Foi identificado que ${h.identificado.trim().replace(/[.]+$/,'')}.`);
  if (h.presentes) parts.push(`Estavam presentes ou relacionados: ${h.presentes.trim().replace(/[.]+$/,'')}.`);
  if (h.providencias) parts.push(`As providências adotadas foram: ${h.providencias.trim().replace(/[.]+$/,'')}.`);
  else if (record.verification.providencias === 'none') parts.push('Não foi necessária providência adicional no momento do registro.');
  if (h.acionados) parts.push(`Foram acionados: ${h.acionados.trim().replace(/[.]+$/,'')}.`);
  if (h.desfecho) parts.push(`Ao final, ${h.desfecho.trim().replace(/[.]+$/,'')}.`);
  if (h.adicional) parts.push(`Informação complementar: ${h.adicional.trim().replace(/[.]+$/,'')}.`);
...
function renderReviewStep() {
  ...
  <div class="review-grid">
    ${reviewSection('Dados da ocorrência',0,`<dl class="definition-grid"><div><dt>Número atual</dt><dd>${escapeHtml(r.numero)}</dd></div><div><dt>Situação</dt><dd>${escapeHtml(syncStatusLabel(r))}</dd></div><div><dt>Data e hora</dt><dd>${formatDateOnly(b.data)} às ${escapeHtml(b.hora)}</dd></div><div><dt>Referência</dt><dd>${escapeHtml(reference||'Não informada')}</dd></div><div><dt>Emissor</dt><dd>${escapeHtml(b.nomeEmissor)} • ${escapeHtml(b.matriculaEmissor)}</dd></div><div><dt>Local</dt><dd>${escapeHtml(b.local)} — ${escapeHtml(b.complementoLocal)}</dd></div><div><dt>Diretoria</dt><dd>${escapeHtml(directorate||'Não informada')}</dd></div></dl>`)}
  ...
  <section class="form-card"><p class="eyebrow">Confirmações obrigatórias</p><h2>Responsabilidade pelo registro</h2><div class="confirmation-stack">
    <label class="checkbox-card ${r.acknowledgements.reviewed?'checked':'required-unchecked'}"><input type="checkbox" data-path="acknowledgements.reviewed" required ${r.acknowledgements.reviewed?'checked':''}><span><strong>Revisei todas as informações</strong><span>Conferi dados, pessoas, veículos, materiais, evidências e relato.</span></span></label>
    <label class="checkbox-card ${r.acknowledgements.truthful?'checked':'required-unchecked'}"><input type="checkbox" data-path="acknowledgements.truthful" required ${r.acknowledgements.truthful?'checked':''}><span><strong>As informações correspondem aos fatos disponíveis</strong><span>Confirmo o conteúdo conforme o registro realizado.</span></span></label>
  </div></section>
...
function bindWizardInputs() {
  app.querySelectorAll('[data-path]').forEach(control => {
    ...
    control.addEventListener(eventName, () => {
      ...
      if (control.id === 'bo-reg') value = value.replace(/\D/g,'');
      if (control.id === 'bo-reg') control.value = value;
...
async function handleStepAction(action) {
  ...
  if (action === 'finalize') {
    ...
    const confirmed = await openAppModal({
      ...
      message: 'Depois da confirmação, o registro original será preservado. Complementos e retificações poderão ser adicionados posteriormente.',
    ...
    addAudit(state.current, 'FINALIZAR BOLETIM', 'Registro finalizado pelo emissor.');
    ...
    await openAppModal({
      ...
      details: `<strong>Número atribuído:</strong> ${escapeHtml(state.current.numero)}${isOfficialNumber(state.current.numero)?'':'<br><small>O número oficial será atribuído quando a sincronização for concluída.</small>'}`,
...
function clearFieldError(id){
  if(!id)return;const control=document.getElementById(id);control?.classList.remove('invalid-field');
  const error=document.querySelector(`[data-error-for="${id}"]`);if(error)error.textContent='';
}
function markFieldError(id,message){
  const control=document.getElementById(id);if(control){control.classList.add('invalid-field');(control._modalSelectButton||control).classList.add('invalid-field');}
  const error=document.querySelector(`[data-error-for="${id}"]`);if(error)error.textContent=message;
}
function showValidation(errors){
  errors.forEach(([id,msg])=>markFieldError(id,msg));
  const first=document.getElementById(errors[0]?.[0]);const target=first?first._modalSelectButton||first:null;
  target?.scrollIntoView({behavior:'smooth',block:'center'});target?.focus({preventScroll:true});
  openAppModal({kind:'warning',eyebrow:'Revisão necessária',title:`${errors.length} informação(ões) precisa(m) de atenção`,message:errors.map(([,msg])=>`• ${escapeHtml(msg)}`).join('<br>'),confirmText:'Corrigir campos'});
}
function dateTimeInFuture(date,time){
  if(!date||!time)return false;const value=new Date(`${date}T${time}:00`);return value.getTime()>Date.now()+60000;
}
function validateStep(step) {
  const errors=[];
  if(step===0){
    const b=state.current.basic;
    if(!b.data)errors.push(['bo-date','Informe a data da ocorrência.']);
    if(!b.hora)errors.push(['bo-time','Informe a hora da ocorrência.']);
    if(b.data&&b.hora&&dateTimeInFuture(b.data,b.hora))errors.push(['bo-time','A data e a hora não podem estar no futuro.']);
    if(!b.referencia)errors.push(['bo-ref','Selecione a referência da ocorrência.']);
    if(b.referencia==='Danos materiais — Outra'&&!b.referenciaOutra.trim())errors.push(['bo-ref-other','Descreva a referência não listada.']);
    if(!/^\d+$/.test(b.matriculaEmissor||''))errors.push(['bo-reg','A matrícula deve conter somente números.']);
    if(!b.nomeEmissor?.trim())errors.push(['bo-name','Informe o nome completo do emissor.']);
    if(b.emailEmissor&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(b.emailEmissor))errors.push(['bo-email','Informe um e-mail válido.']);
    if(!b.local)errors.push(['bo-local','Selecione o tipo de local.']);
    if(!b.complementoLocal?.trim())errors.push(['bo-local-detail','Identifique o local com detalhes.']);
    if(!b.diretoria)errors.push(['bo-directorate','Selecione a diretoria relacionada.']);
    if(b.diretoria==='Outra'&&!b.diretoriaOutra.trim())errors.push(['bo-directorate-other','Informe o nome da diretoria.']);
    if(errors.length){showValidation(errors);return false;}
    rememberLocation(b.complementoLocal);
  }
  if(step===1){
    const v=state.current.verification;
    const pending=['people','witnesses','vehicles','materials'].filter(k=>v[k]==='pending');
    if(pending.length){openAppModal({kind:'warning',eyebrow:'Verificação incompleta',title:'Confirme todos os elementos relacionados',message:'Em cada grupo, cadastre os dados ou marque que não se aplica. Também confirme se há testemunhas.',confirmText:'Revisar etapa'});return false;}
    if(v.people==='has'&&!state.current.people.length){openAppModal({kind:'warning',eyebrow:'Pessoa não cadastrada',title:'Adicione pelo menos uma pessoa',message:'Você marcou que há pessoa identificada, mas nenhum cadastro foi incluído.',confirmText:'Adicionar pessoa'});return false;}
    if(v.vehicles==='has'&&!state.current.vehicles.length){openAppModal({kind:'warning',eyebrow:'Veículo não cadastrado',title:'Adicione pelo menos um veículo',message:'Você marcou que há veículo relacionado, mas nenhum cadastro foi incluído.',confirmText:'Adicionar veículo'});return false;}
    if(v.materials==='has'&&!state.current.materials.length){openAppModal({kind:'warning',eyebrow:'Material não cadastrado',title:'Adicione pelo menos um material',message:'Você marcou que há material relacionado, mas nenhum cadastro foi incluído.',confirmText:'Adicionar material'});return false;}
    if(v.witnesses==='has'&&!state.current.people.some(p=>p.tipo==='Testemunha')){openAppModal({kind:'warning',eyebrow:'Testemunha não cadastrada',title:'Adicione uma testemunha',message:'Você marcou que existem testemunhas, mas não há cadastro com o tipo Testemunha.',confirmText:'Adicionar testemunha'});return false;}
    if(v.people==='none'&&state.current.people.length){openAppModal({kind:'warning',eyebrow:'Informação contraditória',title:'Pessoas cadastradas e “não se aplica” marcado',message:'Revise os cadastros de pessoas.',confirmText:'Revisar'});return false;}
    if(v.witnesses==='none'&&state.current.people.some(p=>p.tipo==='Testemunha')){openAppModal({kind:'warning',eyebrow:'Informação contraditória',title:'Há testemunha cadastrada',message:'Remova a testemunha ou altere a confirmação.',confirmText:'Revisar'});return false;}
  }
  if(step===2){
    const h=state.current.history;
    if(!h.inicio.trim())errors.push(['history-start','Explique como a ocorrência começou.']);
    if(!h.identificado.trim())errors.push(['history-found','Informe o que foi identificado.']);
    if(!h.desfecho.trim())errors.push(['history-end','Informe como a situação terminou.']);
    if(state.current.verification.providencias==='pending')errors.push(['history-actions','Confirme se houve providência.']);
    if(state.current.verification.providencias==='has'&&!h.providencias.trim())errors.push(['history-actions','Descreva as providências adotadas.']);
    if(!h.relato.trim())errors.push(['history-report','Gere ou escreva o relato consolidado.']);
    if(errors.length){showValidation(errors);return false;}
  }
  if(step===3&&state.current.verification.attachments==='has'&&!state.current.attachments.length){openAppModal({kind:'warning',eyebrow:'Anexo não incluído',title:'Adicione uma evidência',message:'Você marcou que há foto ou documento, mas nenhum arquivo foi adicionado.',confirmText:'Adicionar anexo'});return false;}
  if(step===3&&state.current.verification.attachments==='pending'){openAppModal({kind:'warning',eyebrow:'Evidências não verificadas',title:'Informe se existem anexos',message:'Adicione fotos ou documentos, ou confirme que não há anexo disponível.',confirmText:'Revisar evidências'});return false;}
  if(step===4){
    if(!reviewChecklistComplete()){openAppModal({kind:'warning',eyebrow:'Pendências no boletim',title:'Ainda há itens obrigatórios pendentes',message:'Confira a lista de verificação no topo da revisão.',confirmText:'Ver pendências'});return false;}
    if(!state.current.acknowledgements.reviewed||!state.current.acknowledgements.truthful){refreshRequiredFieldStates(app);focusFirstMissing(app);openAppModal({kind:'warning',eyebrow:'Confirmações obrigatórias',title:'Marque as duas confirmações',message:'A finalização será liberada após a revisão e a declaração de correspondência dos fatos.',confirmText:'Revisar confirmações'});return false;}
  }
  return true;
}
function reviewChecklistComplete(){const r=state.current,b=r.basic,v=r.verification;return Boolean(b.data&&b.hora&&b.referencia&&b.matriculaEmissor&&b.nomeEmissor&&b.local&&b.complementoLocal&&b.diretoria&&v.people!=='pending'&&v.witnesses!=='pending'&&v.vehicles!=='pending'&&v.materials!=='pending'&&v.attachments!=='pending'&&v.providencias!=='pending'&&r.history.relato?.trim());}
function validateAll(){return [0,1,2,3,4].every(validateStep);}
...
function renderDetail() {
  ...
  <div class="review-grid">
    ${reviewSectionStatic('Ocorrência',`<dl class="definition-grid"><div><dt>Referência</dt><dd>${escapeHtml(reference||'Não informada')}</dd></div><div><dt>Local</dt><dd>${escapeHtml(b.local)} — ${escapeHtml(b.complementoLocal)}</dd></div><div><dt>Diretoria</dt><dd>${escapeHtml(directorate||'Não informada')}</dd></div><div><dt>Emissor</dt><dd>${escapeHtml(b.nomeEmissor)} • ${escapeHtml(b.matriculaEmissor)}</dd></div><div><dt>Criado</dt><dd>${formatDateTime(r.createdAt)}</dd></div><div><dt>Finalizado</dt><dd>${formatDateTime(r.finalizedAt)}</dd></div></dl>`)}
...
function renderAbout() {
  app.innerHTML = `
    ...
    <div class="about-list">
      ...
      <div class="code-note">Abas criadas: BO_Ocorrencias, BO_Pessoas, BO_Veiculos, BO_Materiais, BO_Anexos, BO_Complementos e BO_Sequencia.</div>
    </div>
...
async function init() {
  try {
    loadSettings();
    state.profile = loadEmitterProfile();
