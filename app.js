...
function renderHome() {
  ...
  app.innerHTML = `
    ...
    <div class="card-grid">
      ...
      <button class="action-card" data-tone="blue" type="button" data-action="install-app">
        <span class="card-icon">${ICONS.download}</span>
        <span><strong>Instalar aplicativo</strong><span>Instale o BO GSP como aplicativo no smartphone.</span></span>
      </button>
    </div>
...
function updateInstallButton() {
  installButton.classList.toggle('hidden', isStandaloneMode());
}
...
async function handleInstallRequest() {
  ...
  if (state.deferredInstall) {
    ...
    if (choice.outcome === 'accepted') {
      await openAppModal({ kind: 'success', eyebrow: 'Instalação', title: 'Instalação iniciada', message: 'O aplicativo será adicionado à lista de aplicativos do aparelho. Em alguns modelos, o atalho também aparece automaticamente na tela inicial.', confirmText: 'Concluir' });
    } else {
      await openAppModal({ kind: 'info', eyebrow: 'Instalação', title: 'Instalação não concluída', message: 'Você pode tentar novamente pelo botão “Instalar” ou pelo menu do navegador.', confirmText: 'Entendi' });
    }
    updateInstallButton();
    return;
  }
  ...
  if (isIOS) {
    await openAppModal({
      kind: 'info',
      eyebrow: 'Instalação no iPhone ou iPad',
      title: 'Instale pelo Safari',
      message: 'Abra este endereço no Safari, toque no botão de compartilhar e escolha “Adicionar à Tela de Início”.',
      details: 'No iOS, a instalação de aplicativos web é controlada pelo sistema e ocorre pela Tela de Início. O comportamento pode variar conforme a versão do iOS.',
      confirmText: 'Entendi'
    });
    return;
  }
  if (isAndroid) {
    await openAppModal({
      ...
      message: 'No Chrome, Edge ou Samsung Internet, abra o menu do navegador e escolha “Instalar aplicativo” ou “Adicionar aplicativo”.',
      details: 'Quando instalado como PWA, o BO GSP aparece junto aos demais aplicativos do aparelho. A opção pode não aparecer em navegadores internos de WhatsApp, Instagram ou outros aplicativos.',
      confirmText: 'Entendi'
    });
    return;
  }
  await openAppModal({
    kind: 'info',
    eyebrow: 'Instalação do aplicativo',
    title: 'Instale pelo navegador',
    message: 'Abra o menu do navegador e procure por “Instalar aplicativo”, “Adicionar aplicativo” ou “Adicionar à tela inicial”.',
    confirmText: 'Entendi'
  });
}
...
window.addEventListener('appinstalled', async () => {
  updateInstallButton();
  await openAppModal({ kind: 'success', eyebrow: 'Instalação concluída', title: 'BO Digital GSP instalado', message: 'O aplicativo foi instalado e pode ser aberto junto aos demais aplicativos compatíveis do aparelho.', confirmText: 'Concluir' });
});
...
async function init() {
  try {
    ...
    updateInstallButton();
    renderHome();
    if ('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v=11', { updateViaCache: 'none' });
  } catch (error) {
    console.error(error);
