...
function dynamicRequirementLabel(key, record = state.current) {
  const fixed = {
    ...
    documents:'Documento da operação', requester:'Solicitante', 'round-operator':'Vigilante responsável pela ronda'
  };
...
function renderBasicStep() {
  ...
  return `
    ...
    ${round ? roundResponsibleCard(state.current) : `<section class="form-card compact-card ${dynamicRequirementActive('requester')?'dynamic-required':''}"><p class="eyebrow">Solicitante</p><h2>Quem acionou a Segurança?</h2><div class="form-grid">${basicRequesterFields(b)}</div></section>`}
...
function renderTemplateQuestion(question, record = state.current) {
  ...
  return `<div class="field full template-question ${level} ${dynamicRequirementActive(`template:${question.id}`)?'dynamic-required':''}" data-template-question="${escapeHtml(question.id)}"><div class="template-question-label"><label class="${question.required?'required':''}" for="${id}">${escapeHtml(question.label)}</label><span class="question-level ${level}">${levelLabel}</span></div>${control}${question.help?`<small>${escapeHtml(question.help)}</small>`:''}<span class="field-error" data-error-for="${id}"></span>${unavailable}</div>`;
}
