...
async function registerOperatorAccess(operator = state.operator) {
  if (!operator || !apiConfigured() || !navigator.onLine) return false;
  storageSet(PENDING_LOGIN_KEY, JSON.stringify(operator));
  try {
    await apiPost({ action: 'login', operator });
    storageRemove(PENDING_LOGIN_KEY);
    return true;
  } catch (error) {
...
function normalizeRecord(record) {
  ...
  normalized.operator ||= {};
  normalized.operator.usuario ||= '';
  normalized.operator.registro ||= '';
  normalized.operator.turno ||= '';
  normalized.operator.loginAt ||= '';
  normalized.operator.accessId ||= '';
  normalized.operator.dispositivo ||= '';
  normalized.history ||= {};
  normalized.history = {
...
async function createNewBo() {
  ...
  record.operator = { ...(state.operator || {}), loginAt: state.operator?.loginAt || new Date().toISOString() };
  await dbPut(record);
  await refreshRecords();
...
async function openRecord(id) {
  ...
  if(!record)return;
  if (!record.operator?.registro && state.operator) record.operator = { ...state.operator };
...
async function handleStepAction(action) {
  ...
  if (action === 'finalize') {
    ...
    if (!confirmed) return;
    showBusy('Aguarde o registro', 'Salvando o boletim no aparelho e preparando o envio para a planilha.');
    let synced = false;
    try {
      state.current.status = 'Finalizado';
      addAudit(state.current, 'FINALIZAR BOLETIM', 'Registro finalizado pelo vigilante responsável pelo preenchimento.');
      state.current.finalizedAt = new Date().toISOString();
      state.current.currentStep = STEPS.length - 1;
      await saveCurrent(true);
      updateBusy('Aguarde o registro', apiConfigured() ? 'Enviando os dados para o Google Sheets. Este processo pode levar alguns segundos.' : 'Concluindo o registro no aparelho.');
      synced = apiConfigured() ? await syncRecord(state.current, false) : false;
      await refreshRecords();
    ...
    await openAppModal({
      ...
      message: synced ? 'O boletim foi salvo no dispositivo e enviado ao Google Sheets.' : 'O boletim foi salvo no dispositivo. O envio à planilha continuará pendente e poderá ser repetido quando houver conexão.',
