...
function basicReferenceFields(b) {
  ...
  const optionsForReferences = (references, compactGeneral = false) => references.map(reference => {
    ...
    const generalLabel = compactGeneral ? 'Registro geral' : `${reference} — geral`;
    const general = `<option value="${escapeHtml(reference)}" data-reference="${escapeHtml(reference)}" data-description="${escapeHtml(generalDescription)}" ${currentSelection===reference?'selected':''}>${escapeHtml(generalLabel)}</option>`;
  ...
  const typeOptions = selectedGroup ? optionsForReferences(selectedGroup.options, true) : '';
  const globalTypeOptions = REFERENCE_GROUPS.map(group => `<optgroup label="${escapeHtml(group.label)}">${group.options.map(reference => {
    const submodels = submodelsFor(reference);
    const generalDescription = REFERENCE_DESCRIPTIONS[reference] || '';
    const general = `<option value="${escapeHtml(reference)}" data-reference="${escapeHtml(reference)}" data-description="${escapeHtml(generalDescription)}" ${currentSelection===reference?'selected':''}>${escapeHtml(submodels.length ? `${reference} — geral` : reference)}</option>`;
    const specifics = submodels.map(item => {
      const value = referenceSelectionValue(reference,item.id);
...
function collectStepIssues(step, record = state.current) {
  ...
  if (step === 0) {
    const b = record.basic || {};
    if (requesterPresence(record) !== 'Sim' && requesterPresence(record) !== 'Não') add('bo-requester-presence','Confirme primeiro se existe solicitante.');
    ...
    if (requesterPresence(record) === 'Não' && !String(b.origemOcorrencia||'').trim()) add('bo-origin','Informe como o fato foi identificado pela Segurança.');
