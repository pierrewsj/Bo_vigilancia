...
function assistantChecklist(record = state.current) {
  ...
  add('bo-requester-presence','Existência de solicitante',requesterPresence(r)==='Sim' || requesterPresence(r)==='Não','required',0,'Confirme se existe solicitante.');
...
function buildSelectModalEntries(select) {
  ...
  Array.from(select.children).forEach(child => {
    if (child.tagName === 'OPTGROUP') {
      ...
      entries.push({ type: 'option', value: child.value, label: child.textContent.trim(), disabled: child.disabled, description: child.dataset.description || (select.id === 'bo-ref' ? REFERENCE_DESCRIPTIONS[child.dataset.reference || child.value] : '') });
    }
  });
  return entries;
}
...
function basicReferenceFields(b) {
  ...
  const typeOptions = selectedGroup ? selectedGroup.options.map(reference => {
    ...
    const general = `<option value="${escapeHtml(reference)}" data-reference="${escapeHtml(reference)}" data-description="${escapeHtml(generalDescription)}" ${currentSelection===reference?'selected':''}>${escapeHtml(reference)} — geral</option>`;
    ...
    return `<optgroup label="${escapeHtml(reference)}">${general}${specifics}</optgroup>`;
  }).join('') : '';
  return `
    <div class="field full occurrence-classifier">
      <label class="required">1. Categoria</label>
      <p class="field-intro">Escolha a categoria do fato. Em seguida, será aberta uma caixa com somente os tipos de boletim relacionados ao tema selecionado.</p>
    ...
    ${selectedGroup ? `<div class="field full occurrence-type-select"><label class="required" for="bo-ref">2. Tipo de boletim</label><select id="bo-ref" aria-label="Tipo de boletim"><option value="" disabled>Selecione o tipo de boletim</option>${typeOptions}</select><small>Você também pode tocar novamente neste campo para trocar o tipo. As variações operacionais ficam organizadas na mesma caixa de seleção.</small><span class="field-error" data-error-for="bo-ref"></span></div>` : '<div class="notice subtle"><strong>Selecione uma categoria</strong><span>A caixa de tipos de boletim será aberta automaticamente.</span></div>'}
...
function requesterPresenceCard(record = state.current) {
  ...
  return `<section class="form-card compact-card requester-presence-card ${(dynamicRequirementActive('requester')||dynamicRequirementActive('operator-responsible'))?'dynamic-required':''}"><p class="eyebrow">Solicitante</p><h2>Existe solicitante?</h2><div id="bo-requester-presence" class="segmented-control requester-presence-control" role="group" aria-label="Existe solicitante?">
...
function bindStepSpecific() {
  app.querySelectorAll('[data-select-category]').forEach(button => button.addEventListener('click', async()=>{
    ...
    if (changed) {
      ...
      state.current.basic.origemOcorrencia = '';
      state.current.basic.temSolicitante = '';
  ...
  if (referenceSelect) referenceSelect.addEventListener('change', async()=>{
    const parsed = parseReferenceSelection(referenceSelect.value || '');
    ...
    const sub = selectedSubmodel(state.current);
    state.current.basic.origemOcorrencia = sub?.origin || defaultOriginForReference(reference);
    if (sub?.routingAnswers) Object.entries(sub.routingAnswers).forEach(([key,val])=>{ state.current.templateData.answers[key]=val; });
    if (ROUND_ORIGINS.has(state.current.basic.origemOcorrencia)) {
      state.current.basic.temSolicitante = 'Não';
      clearRequesterData(state.current);
      state.current.history.fonteRelato = 'Constatação da equipe em ronda';
    }
  ...
  app.querySelectorAll('[data-requester-presence]').forEach(button => button.addEventListener('click', async()=>{
    ...
    if (value === 'Não') {
      if (state.current.basic.origemOcorrencia === 'Solicitação recebida') state.current.basic.origemOcorrencia = 'Constatação espontânea da Segurança';
      clearRequesterData(state.current);
    } else if (!state.current.basic.origemOcorrencia || state.current.basic.origemOcorrencia === 'Constatação espontânea da Segurança') {
...
function collectStepIssues(step, record = state.current) {
  ...
  if (step === 0) {
    ...
    if (requesterPresence(record) !== 'Sim' && requesterPresence(record) !== 'Não') add('bo-requester-presence','Confirme se existe solicitante.');
    ...
    } else if (recordWithoutRequester(record)) {
...
function buildPrintDocument(record) {
  ...
  const materials = (r.materials||[]).map(m=>`<article><strong>${printValue(m.denominacao,'Item sem identificação técnica')}</strong><span>${[m.quantidade?`${m.quantidade} ${m.unidade||''}`:'',m.desenho,m.fornecedor,m.codigoVasilhame].filter(Boolean).map(escapeHtml).join(' • ')}</span></article>`);
