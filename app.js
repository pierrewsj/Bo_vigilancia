  ...
  return `<div class="review-professional-layout v32-review">
    ...
    <div class="review-detail-panel">
      ...
      <div class="review-grid compact-review-grid">
        ...
        ${reviewSection('Relato consolidado',4,`<div class="final-report-preview">${escapeHtml(r.history.relato||'Relato ainda não gerado.').replace(/\n/g,'<br>')}</div><div class="about-actions"><button class="button secondary small" type="button" data-action="assistant-generate">${ICONS.sync} Atualizar relato</button><button class="button secondary small" type="button" data-action="assistant-review-text">${ICONS.check} Revisar redação</button></div>`)}
      </div>
...
function renderReportCorrection() {
  ...
  </div><div class="correction-inline-action"><button class="button primary" type="button" data-action="assistant-generate">${ICONS.file} Gerar ou atualizar relato</button><button class="button secondary" type="button" data-action="assistant-review-text">${ICONS.check} Revisar redação</button></div></section>`;
}
...
Informe uma justificativa curta:`) || '').trim();
  ...
  app.querySelectorAll('[data-action="assistant-review-text"]').forEach(reviewButton => reviewButton.addEventListener('click', async () => {
    ...
    addAudit(state.current, 'REVISÃO TEXTUAL NEXO', changed ? 'Redação revisada: conectores, repetições, pontuação e padronização linguística.' : 'Texto conferido; nenhuma correção automática adicional foi necessária.');
    await saveCurrent(true, false);
    renderWizard();
    showToast(changed ? 'Redação revisada pelo NEXO.' : 'Texto conferido: nenhuma correção adicional.');
  }));
