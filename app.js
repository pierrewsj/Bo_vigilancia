...
const OPERATOR_SESSION_KEY = 'bo-digital-operator-session-v1';
...
function loadOperatorSession() {
  try {
    const operator = JSON.parse(sessionGet(OPERATOR_SESSION_KEY) || 'null');
    return operator && operator.usuario && operator.registro && operator.turno ? operator : null;
  } catch { return null; }
}
function saveOperatorSession(operator) {
  sessionSet(OPERATOR_SESSION_KEY, JSON.stringify(operator));
  state.operator = operator;
}
function clearOperatorSession() {
  sessionRemove(OPERATOR_SESSION_KEY);
  state.operator = null;
}
...
function renderLogin() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (apiConfigured() && navigator.onLine) {
      apiPost({ action: 'login', operator: operatorData })
        .catch(error => console.warn('Login não registrado na planilha neste momento.', error));
    }
  });
}
...
async function init() {
  try {
    ...
    await refreshRecords();
    state.route = state.operator ? 'home' : 'login';
    updateHeader();
    updateInstallButton();
    await render();
