  ...
  app.innerHTML = `
    <section class="hero">
      ...
      <div class="hero-visual" aria-hidden="true">
        <div class="hero-status"><span class="chip guardiao-online">Guardião online</span></div>
        <img src="assets/guardiao-gsp-hero.webp" alt="">
      </div>
    </section>
    ...
    <div class="card-grid">
      ...
      <button class="action-card" type="button" data-action="open-bot">
        ...
        <span><strong>Assistente Guardião</strong><span>Receba orientação e confira pendências da etapa atual.</span></span>
      </button>
    </div>
...
function renderDetail() {
  ...
  app.innerHTML = `
    <section class="hero">
      ...
      <div class="hero-visual" aria-hidden="true"><img src="assets/guardiao-gsp-hero.webp" alt=""></div>
    </section>
...
function botContextText() {
  if (state.route === 'home') return 'Olá! Sou o Guardião GSP. Posso orientar cada etapa, indicar os campos obrigatórios pendentes e explicar como registrar pessoas, veículos, materiais, anexos e o histórico da ocorrência.';
  if (state.route === 'records') return 'Nesta tela você pode pesquisar pelo número do BO, referência, emissor, pessoa ou placa. Também é possível continuar os rascunhos salvos.';
  if (state.route === 'about') return 'Para conectar o Google Sheets, publique o arquivo google-apps-script.gs como aplicativo da Web e cole o endereço da implantação terminado em /exec.';
  if (state.route === 'detail') return 'Este boletim está finalizado. Você pode imprimir, salvar em PDF ou sincronizar novamente com a planilha.';
  if (state.route === 'wizard') {
    const tips = [
      'Preencha os campos destacados em amarelo. As antigas sub-referências estão incorporadas à lista de Referência.',
      'Adicione solicitantes, envolvidos e testemunhas. Quando ninguém for identificado, marque a caixa correspondente.',
      'Cadastre cada veículo separadamente e relacione-o a uma pessoa quando essa ligação existir.',
      'Cadastre materiais, MVM ou nota fiscal. Um mesmo boletim pode possuir vários registros de materiais.',
      'Os anexos são opcionais. Nesta versão, os arquivos ficam no dispositivo e a planilha recebe apenas seus dados básicos.',
      'Descreva os fatos em ordem cronológica: início, constatação, providências adotadas e desfecho.',
      'Confira cada seção e marque as duas caixas de confirmação para liberar a finalização.'
    ];
...
function botResponse(question) {
  ...
  if (q.includes('falta') || q.includes('obrigat')) {
    if (state.route === 'wizard' && state.currentStep === 0) {
      ...
      return missing.length ? `Ainda falta preencher: ${missing.join(', ')}.` : 'Os dados principais obrigatórios estão completos. Você pode continuar para a próxima etapa.';
    }
    if (state.route === 'wizard' && state.currentStep === 5 && !state.current.history.relato.trim()) return 'Ainda falta preencher o relato completo da ocorrência.';
    if (state.route === 'wizard' && state.currentStep === 6 && (!state.current.acknowledgements.reviewed || !state.current.acknowledgements.truthful)) return 'Na revisão final, marque as duas caixas de confirmação obrigatórias.';
    return 'Os campos obrigatórios vazios aparecem em amarelo. Depois de preenchidos, recebem um destaque verde.';
  }
  if (q.includes('refer')) return 'A antiga sub-referência foi incorporada à Referência. Exemplo: “Danos materiais — Entrada com danos”. Assim, a classificação é feita em uma única seleção.';
  if (q.includes('diretoria')) return 'Selecione “Outra” em Diretoria relacionada. O aplicativo exibirá um novo campo para digitar o nome da diretoria.';
  ...
  if (q.includes('sem vínculo')) return 'Ao escolher “Sem vínculo”, aparecem os campos de documento e dados complementares. Preencha somente as informações necessárias para o processo.';
  if (q.includes('sem pessoa')) return 'Quando ninguém for identificado, marque a caixa “Não há pessoa identificada”. Não deixe essa caixa marcada se houver pessoas cadastradas.';
  if (q.includes('anexo') || q.includes('foto')) return 'Na etapa Anexos, toque na área de seleção e escolha fotos ou documentos. Cada arquivo pode ter até 5 MB neste protótipo.';
  if (q.includes('planilha') || q.includes('google') || q.includes('conex')) return 'Abra a aba Banco, cole o endereço do Apps Script terminado em /exec, salve e use “Testar conexão”. O ZIP contém o script e o passo a passo.';
  if (q.includes('sincron')) return 'Você pode sincronizar ao salvar, ao finalizar ou pelo botão “Enviar registros locais” na aba Banco.';
  if (q.includes('rascunho') || q.includes('salvar')) return 'O aplicativo salva o rascunho automaticamente neste dispositivo. O botão “Salvar rascunho” força a gravação imediata e tenta sincronizar quando o banco está configurado.';
  if (q.includes('etapa') || q.includes('explicar')) return botContextText();
  return 'Posso ajudar com campos obrigatórios, referência, diretoria, pessoas, anexos, rascunhos, confirmações finais e Google Sheets.';
}
