...
function materialFieldPolicy(record = state.current, material = {}) {
  ...
  const loadContext = MATERIAL_LOAD_REFERENCES.has(reference);
  ...
  // Campos complementares permanecem disponíveis, porém nunca são exigidos por simples cadastro do item.
  // Em BOs compactos ficam recolhidos em uma seção opcional para não induzir o vigilante a inventar dados.
  const showDocument = true;
  const showSupplier = true;
  const showDrawing = true;
  const showContainer = true;
  let guidance = 'Cadastre a identificação principal do item. Os dados complementares devem ser preenchidos somente quando existirem e forem pertinentes ao fato.';
  if (reference === 'Material ou equipamento localizado/recolhido' || reference === 'Irregularidade constatada em ronda externa' || reference === 'Irregularidade constatada em ronda interna') {
    guidance = 'Para item localizado ou recolhido, priorize a denominação e a condição encontrada. MVM, fornecedor, desenho e código de vasilhame não são exigidos quando não fizerem parte da situação.';
  } else if (loadContext) {
    guidance = 'Neste tipo de operação, documento, quantidade e unidade podem ser relevantes para a conferência. Informe apenas o documento que realmente existir; MVM não deve ser criado ou presumido.';
  }
  return { reference, loadContext, compactContext, quantityRequired, showDocument, showSupplier, showDrawing, showContainer, guidance };
}

function materialSectionDescription(record = state.current) {
  const policy = materialFieldPolicy(record);
  if (policy.compactContext) return 'Cadastre o item encontrado ou relacionado ao fato. Documentos e códigos internos aparecem somente quando forem pertinentes.';
  if (policy.loadContext) return 'Cadastre a carga/material e os dados disponíveis para conferência. MVM só deve ser informado quando esse documento existir na operação.';
...
function assistantChecklist(record = state.current) {
  ...
  add('bo-name','Solicitante identificado',meaningfulText(b.nomeEmissor),'required',0,'Informe o nome do solicitante.');
...
function assistantMaterialSummary(material) {
  ...
  if (material.mvm) details.push(`documento da operação ${material.mvm}`);
...
function buildNarrative(record = state.current) {
  ...
  let introduction = `Em ${formatDateOnly(b.data)}, às ${b.hora}, a equipe de Segurança Patrimonial iniciou atendimento${local ? ` no local ${local}` : ''}`;
  if (reference) introduction += ` para registro de ${lowerInitialPt(reference)}`;
  introduction += '.';
  paragraphs.push(introduction);

  if (meaningfulText(b.nomeEmissor)) {
  ...
  if (found) paragraphs.push(`No local, a equipe de Segurança Patrimonial registrou a seguinte constatação: ${sentenceCasePt(found)}.`);
  ...
  if (record.verification?.attachments === 'none' && meaningfulText(record.templateData?.evidenceUnavailableReason)) {
    paragraphs.push(`Não foi possível registrar evidência fotográfica ou documental. Motivo informado: ${cleanSentence(record.templateData.evidenceUnavailableReason)}.`);
  }
...
function renderInvestigationStep() {
  ...
  <section class="form-card fact-source-card"><div class="fact-source-grid"><div class="field full"><div class="fact-source-title"><span>1</span><div><strong>Relato recebido</strong><small>Informação de solicitante, envolvido ou terceiro</small></div></div><label class="required" for="history-source">Fonte principal do relato</label><select id="history-source" data-path="history.fonteRelato" required>${selectOptions(['Solicitante','Pessoa envolvida','Testemunha','Terceiro/outro informante','Fonte não identificada'],h.fonteRelato||'Solicitante')}</select><small>Escolha quem forneceu a informação principal. O relatório inserirá o conector adequado automaticamente.</small><label class="required" for="history-start">Como o fato foi informado à equipe?</label><textarea id="history-start" data-path="history.inicio" placeholder="Ex.: Uma pessoa teria tentado acessar a área pulando o muro. Escreva somente o conteúdo; o sistema adicionará a fonte do relato." required>${escapeHtml(h.inicio)}</textarea><span class="field-error" data-error-for="history-start"></span></div><div class="field full"><div class="fact-source-title"><span>2</span><div><strong>Constatação da Segurança</strong><small>Somente fatos observados ou verificados</small></div></div><label class="required" for="history-found">O que a equipe constatou no local?</label><textarea id="history-found" data-path="history.identificado" placeholder="Ex.: Foram observadas marcas de barro no muro e não havia pessoas no local. Registre somente o que foi verificado." required>${escapeHtml(h.identificado)}</textarea><span class="field-error" data-error-for="history-found"></span></div></div></section>
  ...
  <div class="notice subtle"><strong>Linguagem profissional:</strong><span>registre fatos observáveis e separe claramente relato de terceiros de constatações da equipe. Evite conclusões sobre culpa, intenção, diagnóstico ou prática de ilícito sem confirmação. Quando uma informação não puder ser obtida, use “Não foi possível obter” e informe o motivo.</span></div>`;
}
...
function renderMaterialItem(material, index) {
  return `<article class="entity-item">
    <div><h3>${escapeHtml(material.denominacao || 'Material sem denominação')}</h3><div class="entity-meta">${material.quantidade ? `<span class="chip">${escapeHtml(material.quantidade)} ${escapeHtml(material.unidade)}</span>` : ''}${material.mvm ? `<span class="chip">Documento ${escapeHtml(material.mvm)}</span>` : ''}${material.pessoaNome ? `<span class="chip">Ligado a ${escapeHtml(material.pessoaNome)}</span>` : ''}</div></div>
  ...
  return `<div class="review-professional-layout v32-review">
    ...
    <div class="review-detail-panel">
      ...
      <div class="review-grid compact-review-grid">
        ...
        ${reviewSection('Apuração padronizada',2,`<dl class="definition-grid"><div style="grid-column:1/-1"><dt>Relato recebido • ${escapeHtml(r.history.fonteRelato||'Solicitante')}</dt><dd>${escapeHtml(r.history.inicio||'Não informado')}</dd></div><div style="grid-column:1/-1"><dt>Constatação da Segurança</dt><dd>${escapeHtml(r.history.identificado||'Não informado')}</dd></div>${answers.map(item=>`<div><dt>${escapeHtml(item.label)}</dt><dd>${escapeHtml(item.value)}</dd></div>`).join('')}</dl>`)}
...
function renderMaterialItemReview(m) { const qty=m.quantidade?`<span class="chip">${escapeHtml(m.quantidade)} ${escapeHtml(m.unidade||'')}</span>`:''; return `<div><strong>${escapeHtml(m.denominacao||'Item sem identificação técnica')}</strong><div class="entity-meta">${qty}</div></div>`; }
...
function materialForm(m) {
  ...
  const quantityRequired = policy.quantityRequired;
  const operationDocument = m.mvm || (policy.loadContext ? String(state.current.templateData?.answers?.documento || '') : '');
  const complementaryFields = [
    policy.showDocument ? `<div class="field"><label for="material-mvm">Documento da operação</label><input id="material-mvm" name="mvm" value="${escapeHtml(operationDocument)}" placeholder="MVM, NF, romaneio, quando existir"><small>MVM somente quando aplicável à movimentação.</small></div>` : '',
    ...
    policy.showDrawing ? `<div class="field"><label for="material-drawing">Desenho / código da peça</label><input id="material-drawing" name="desenho" value="${escapeHtml(m.desenho||'')}" placeholder="Quando disponível"></div>` : '',
    policy.showContainer ? `<div class="field"><label for="material-container">Código do vasilhame</label><input id="material-container" name="codigoVasilhame" value="${escapeHtml(m.codigoVasilhame||'')}" placeholder="Quando aplicável"></div>` : ''
  ].filter(Boolean).join('');
  return `<div class="entity-context-note"><strong>Cadastro adaptado ao tipo de BO</strong><span>${escapeHtml(policy.guidance)}</span></div><div class="form-grid">
    <div class="field full"><label class="required" for="material-name">Denominação</label><input id="material-name" name="denominacao" value="${escapeHtml(m.denominacao||'')}" required placeholder="Nome da peça, material ou equipamento"><button type="button" class="field-helper-button" data-material-unidentified>Item sem identificação técnica</button></div>
    <div class="field"><label class="${quantityRequired?'required':''}" for="material-quantity">Quantidade</label><input id="material-quantity" type="number" min="0.01" step="0.01" name="quantidade" value="${escapeHtml(m.quantidade||'')}" ${quantityRequired?'required':''} placeholder="Quando possível"><small>${quantityRequired?'Essencial para esta conferência.':'Opcional quando não for possível determinar com segurança.'}</small></div>
    <div class="field"><label class="${quantityRequired?'required':''}" for="material-unit">Unidade</label><input id="material-unit" name="unidade" value="${escapeHtml(m.unidade||'')}" placeholder="un, kg, cx..." ${quantityRequired?'required':''}><small>Obrigatória somente quando houver quantidade informada.</small></div>
    ${complementaryFields ? `<div class="field full material-complementary-wrap"><details class="material-complementary" ${policy.loadContext?'open':''}><summary>Identificação complementar <span>quando disponível</span></summary><div class="form-grid">${complementaryFields}</div></details></div>` : ''}
    ...
    <div class="field full"><label for="material-notes">Observações</label><textarea id="material-notes" name="observacao" placeholder="Condição encontrada, características, local ou informação que ajude a identificar o item.">${escapeHtml(m.observacao||'')}</textarea></div>
  </div>`;
}
...
function bindDialogDynamic(type) {
  ...
  if(type==='materials'){
    ...
    const qty=dialogBody.querySelector('#material-quantity'),unit=dialogBody.querySelector('#material-unit');
    const update=()=>{
      if(unit)unit.required=policy.quantityRequired || Boolean(String(qty?.value||'').trim());
      refreshRequiredFieldStates(dialogBody);
    };
    ...
    dialogBody.querySelector('[data-material-unidentified]')?.addEventListener('click',()=>{
      ...
      refreshRequiredFieldStates(dialogBody);showToast('Item registrado sem identificação técnica.');
    });
  }
}
...
async function saveDialogEntity(event) {
  ...
  if(type==='materials'){
    ...
    if(policy.loadContext&&!String(formData.mvm||'').trim()) formData.mvm=String(state.current.templateData?.answers?.documento||'').trim();
    ...
    if(hasQuantity&&!String(formData.unidade||'').trim()){await openAppModal({kind:'warning',eyebrow:'Unidade necessária',title:'Informe a unidade da quantidade',message:'Ex.: un, kg, cx. Se a quantidade não puder ser determinada, deixe quantidade e unidade em branco.',confirmText:'Completar cadastro'});return;}
    formData.pessoaNome=personNameById(formData.pessoaId);
  }
...
function renderDetail() {
  ...
  <div class="review-grid">
    ...
    ${reviewSectionStatic('Apuração padronizada',`<dl class="definition-grid"><div style="grid-column:1/-1"><dt>Relato recebido • ${escapeHtml(r.history.fonteRelato||'Solicitante')}</dt><dd>${escapeHtml(r.history.inicio||'Não informado')}</dd></div><div style="grid-column:1/-1"><dt>Constatação da Segurança</dt><dd>${escapeHtml(r.history.identificado||'Não informado')}</dd></div>${templateAnswerSummary(r).map(item=>`<div><dt>${escapeHtml(item.label)}</dt><dd>${escapeHtml(item.value)}</dd></div>`).join('')}</dl>`)}
