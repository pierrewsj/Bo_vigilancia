...
async function refreshRecords() {
  state.records = (await dbGetAll()).sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));
}
...
function renderHome() {
  ...
  app.innerHTML = `
    <section class="hero">
      ...
      <p>Cadastre a ocorrência por etapas, inclua quantas pessoas, veículos e materiais forem necessários e revise tudo antes de finalizar.</p>
...
function recordCard(record) {
  const summary = record.basic?.referencia || 'Ocorrência ainda não classificada';
  return `
...
function renderBasicStep() {
  ...
  const subOptions = SUBREFERENCES[b.referencia] || ['Outra'];
  const anotherVisible = b.subreferencia === 'Outra';
  return `
    <section class="form-card">
      <p class="eyebrow">Etapa 1</p>
      <h1>Dados da ocorrência</h1>
      <p>Informe os dados principais. Data e hora já vêm preenchidas, mas podem ser corrigidas.</p>
      <div class="form-grid">
        <div class="field">
          <label class="required" for="bo-date">Data da ocorrência</label>
          <input id="bo-date" type="date" value="${escapeHtml(b.data)}" data-path="basic.data">
        </div>
        <div class="field">
          <label class="required" for="bo-time">Hora da ocorrência</label>
          <input id="bo-time" type="time" value="${escapeHtml(b.hora)}" data-path="basic.hora">
        </div>
        <div class="field">
          <label class="required" for="bo-ref">Referência</label>
          <select id="bo-ref" data-path="basic.referencia" data-rerender="true">${selectOptions(REFERENCES, b.referencia)}</select>
        </div>
        <div class="field">
          <label class="required" for="bo-subref">Sub-referência</label>
          <select id="bo-subref" data-path="basic.subreferencia" data-rerender="true" ${b.referencia ? '' : 'disabled'}>${selectOptions(subOptions, b.subreferencia, b.referencia ? 'Selecione' : 'Escolha a referência primeiro')}</select>
        </div>
        ${anotherVisible ? `<div class="field full"><label class="required" for="bo-subref-other">Descreva a sub-referência</label><input id="bo-subref-other" type="text" value="${escapeHtml(b.subreferenciaOutra)}" data-path="basic.subreferenciaOutra" placeholder="Informe a descrição complementar"></div>` : ''}
        <div class="field">
          <label class="required" for="bo-reg">Matrícula do emissor</label>
          <input id="bo-reg" type="text" inputmode="numeric" value="${escapeHtml(b.matriculaEmissor)}" data-path="basic.matriculaEmissor" placeholder="Ex.: 76313">
        </div>
        <div class="field">
          <label class="required" for="bo-name">Nome do emissor</label>
          <input id="bo-name" type="text" value="${escapeHtml(b.nomeEmissor)}" data-path="basic.nomeEmissor" autocomplete="name">
        </div>
        ...
        <div class="field">
          ...
          <select id="bo-local" data-path="basic.local">${selectOptions(LOCATIONS, b.local)}</select>
        </div>
        <div class="field">
          <label class="required" for="bo-local-detail">Descrição complementar do local</label>
          <input id="bo-local-detail" type="text" value="${escapeHtml(b.complementoLocal)}" data-path="basic.complementoLocal" placeholder="Numeração, sala, rua, almoxarifado...">
        </div>
        <div class="field full">
          <label class="required" for="bo-directorate">Diretoria relacionada</label>
          <select id="bo-directorate" data-path="basic.diretoria">${selectOptions(DIRECTORATES, b.diretoria)}</select>
        </div>
      </div>
    </section>
    <div class="notice">As opções completas de sub-referência deverão ser copiadas do Forms. Neste protótipo, a ramificação de “Danos materiais” já está demonstrada.</div>`;
}
...
function renderHistoryStep() {
  ...
  return `
    <section class="form-card">
      ...
      <div class="form-grid">
        <div class="field full"><label class="required" for="history-report">Relato completo</label><textarea id="history-report" data-path="history.relato" placeholder="Descreva o início, meio e fim da ocorrência...">${escapeHtml(h.relato)}</textarea></div>
    ...
    <div class="review-grid">
      ${reviewSection('Dados da ocorrência', 0, `
        <dl class="definition-grid">
          ...
          <div><dt>Referência</dt><dd>${escapeHtml(b.referencia)}</dd></div>
          <div><dt>Sub-referência</dt><dd>${escapeHtml(b.subreferencia === 'Outra' ? b.subreferenciaOutra : b.subreferencia)}</dd></div>
          ...
          <div><dt>Diretoria</dt><dd>${escapeHtml(b.diretoria)}</dd></div>
        </dl>`)}
...
function bindWizardInputs() {
  app.querySelectorAll('[data-path]').forEach(control => {
    ...
    control.addEventListener(eventName, () => {
      ...
      if (control.dataset.path === 'basic.referencia') {
        state.current.basic.subreferencia = '';
        state.current.basic.subreferenciaOutra = '';
      }
      scheduleSave();
...
async function handleStepAction(action) {
  if (action === 'save') return saveCurrent(false);
  if (action === 'exit') {
    await saveCurrent(true);
  ...
  if (action === 'finalize') {
    ...
    await saveCurrent(true);
    showToast('Boletim finalizado com sucesso.');
    navigate('detail');
  }
}
...
function validateStep(step) {
  if (step === 0) {
    const b = state.current.basic;
    const missing = !b.data || !b.hora || !b.referencia || !b.subreferencia || !b.matriculaEmissor || !b.nomeEmissor || !b.local || !b.complementoLocal || !b.diretoria || (b.subreferencia === 'Outra' && !b.subreferenciaOutra);
    if (missing) { showToast('Preencha todos os campos obrigatórios da ocorrência.'); return false; }
  }
...
function renderRecords() {
  ...
  const filtered = state.records.filter(record => {
    ...
    const haystack = [record.numero, record.basic?.referencia, record.basic?.nomeEmissor, record.basic?.local, ...(record.people || []).map(p => p.nome), ...(record.vehicles || []).map(v => v.placa)].join(' ').toLowerCase();
...
function renderDetail() {
  ...
  app.innerHTML = `
    <section class="hero">
      ...
      <p>${escapeHtml(b.referencia)} • ${formatDateOnly(b.data)} às ${escapeHtml(b.hora)}</p>
      <div class="hero-actions"><button class="button ghost" type="button" data-action="print">Imprimir / PDF</button></div>
    </section>
    <div class="section-title"><div><p class="eyebrow">Resumo</p><h2>Dados do boletim</h2></div></div>
    <div class="review-grid">
      ${reviewSectionStatic('Ocorrência', `<dl class="definition-grid"><div><dt>Sub-referência</dt><dd>${escapeHtml(b.subreferencia === 'Outra' ? b.subreferenciaOutra : b.subreferencia)}</dd></div><div><dt>Local</dt><dd>${escapeHtml(b.local)} — ${escapeHtml(b.complementoLocal)}</dd></div><div><dt>Diretoria</dt><dd>${escapeHtml(b.diretoria)}</dd></div><div><dt>Emissor</dt><dd>${escapeHtml(b.nomeEmissor)} (${escapeHtml(b.matriculaEmissor)})</dd></div><div><dt>Criado em</dt><dd>${formatDateTime(r.createdAt)}</dd></div><div><dt>Finalizado em</dt><dd>${formatDateTime(r.finalizedAt)}</dd></div></dl>`)}
...
function renderAbout() {
  app.innerHTML = `
    <section class="form-card">
      <p class="eyebrow">Protótipo PWA</p><h1>Como esta versão funciona</h1>
      <p>Ela foi criada para validar o fluxo antes da construção definitiva no Power Apps.</p>
      <div class="notice warning">Use apenas dados fictícios. Os registros e anexos ficam armazenados localmente neste navegador e não possuem a segurança, permissões ou auditoria do ambiente corporativo.</div>
    </section>
    <div class="section-title"><div><p class="eyebrow">Recursos</p><h2>Incluídos nesta versão</h2></div></div>
    <div class="about-list">
      <div class="about-item"><span class="card-icon">${ICONS.phone}</span><div><strong>Instalável no celular</strong><p>Quando publicado em HTTPS, pode ser adicionado à tela inicial como aplicativo.</p></div></div>
      <div class="about-item"><span class="card-icon">${ICONS.database}</span><div><strong>Dados locais</strong><p>Rascunhos, boletins e anexos são mantidos no IndexedDB do navegador.</p></div></div>
      <div class="about-item"><span class="card-icon">${ICONS.shield}</span><div><strong>Não é a versão definitiva</strong><p>O Power Apps final deverá usar autenticação corporativa, SharePoint e permissões controladas.</p></div></div>
    </div>
