...
function renderHome() {
  ...
  app.innerHTML = `
    ...
    <div class="card-grid">
      ...
      ${state.deferredInstall && !isStandaloneMode() ? `<button class="action-card" data-tone="blue" type="button" data-action="install-app">
...
function updateInstallButton() {
  const canInstall = Boolean(state.deferredInstall) && !isStandaloneMode();
...
async function handleInstallRequest() {
  ...
  if (!state.deferredInstall) {
    await openAppModal({
      ...
      message: 'A instalação real só será liberada quando o navegador reconhecer todos os requisitos do aplicativo. Abra o endereço publicado diretamente no Google Chrome, aguarde alguns segundos e atualize a página.',
      details: 'Não use “Adicionar à tela inicial”, pois essa opção pode criar apenas um atalho. O botão Instalar será exibido automaticamente quando a instalação como aplicativo estiver disponível.',
      confirmText: 'Entendi'
    });
    return;
  }
  ...
  if (choice.outcome !== 'accepted') {
    await openAppModal({ kind: 'info', eyebrow: 'Instalação', title: 'Instalação cancelada', message: 'O navegador cancelou a instalação. Recarregue a página para tentar novamente.', confirmText: 'Entendi' });
  }
}
...
async function init() {
  try {
    ...
    updateInstallButton();
    renderHome();
    if ('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js', { scope: './', updateViaCache: 'none' });
  } catch (error) {
    console.error(error);
