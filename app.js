...
function essentialQuestionHelp(question) {
  if (String(question.help || '').trim()) return String(question.help).trim();
  const type = String(question.type || 'text').toLowerCase();
  if (type === 'select') return 'Selecione a opção que melhor corresponde ao que foi constatado neste atendimento.';
  if (type === 'time') return 'Informe o horário efetivamente constatado. Se não for possível obtê-lo, registre a impossibilidade logo abaixo.';
  if (type === 'date') return 'Informe a data correspondente ao fato ou à constatação realizada.';
  if (type === 'number') return 'Informe somente o valor efetivamente verificado; não estime quando não houver base segura.';
  if (type === 'textarea') return 'Descreva de forma curta e objetiva somente o que foi informado, observado ou confirmado.';
  return 'Preencha este dado conforme a informação disponível no local, sem criar ou presumir informações.';
}
...
function renderTemplateQuestion(question, record = state.current) {
  ...
  const helpText = question.required ? essentialQuestionHelp(question) : String(question.help || '').trim();
  return `<div class="field full template-question ${level} ${dynamicRequirementActive(`template:${question.id}`)?'dynamic-required':''}" data-template-question="${escapeHtml(question.id)}"><div class="template-question-label"><label class="${question.required?'required':''}" for="${id}">${escapeHtml(question.label)}</label><span class="question-level ${level}">${levelLabel}</span></div>${control}${helpText?`<small class="${question.required?'essential-field-help':''}">${escapeHtml(helpText)}</small>`:''}<span class="field-error" data-error-for="${id}"></span>${unavailable}</div>`;
}
