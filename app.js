...
const SESSION_KEY = 'bo-digital-user-session-v1';
...
const MENU_FALLBACK = [
  { id: 'novo', titulo: 'Novo boletim', subtitulo: 'Iniciar um novo registro', acao: 'new-bo', icone: 'clipboard', tom: 'blue', ativo: true, ordem: 1 },
  { id: 'rascunho', titulo: 'Continuar rascunho', subtitulo: 'Abrir o rascunho mais recente', acao: 'continue-draft', icone: 'file', tom: 'green', ativo: true, ordem: 2 },
  { id: 'boletins', titulo: 'Consultar boletins', subtitulo: 'Pesquisar registros locais e sincronizados', acao: 'show-records', icone: 'search', tom: 'blue', ativo: true, ordem: 3 },
  { id: 'ocorrencia', titulo: 'Etapa 1 • Ocorrência', subtitulo: 'Ir para os dados principais', acao: 'wizard-step-0', icone: 'shield', tom: 'neutral', ativo: true, ordem: 4 },
  { id: 'pessoas', titulo: 'Etapa 2 • Pessoas', subtitulo: 'Solicitantes, envolvidos e testemunhas', acao: 'wizard-step-1', icone: 'users', tom: 'neutral', ativo: true, ordem: 5 },
  { id: 'veiculos', titulo: 'Etapa 3 • Veículos', subtitulo: 'Cadastrar placas e responsáveis', acao: 'wizard-step-2', icone: 'car', tom: 'neutral', ativo: true, ordem: 6 },
  { id: 'materiais', titulo: 'Etapa 4 • Materiais', subtitulo: 'Cadastrar volumes e itens', acao: 'wizard-step-3', icone: 'box', tom: 'neutral', ativo: true, ordem: 7 },
  { id: 'anexos', titulo: 'Etapa 5 • Anexos', subtitulo: 'Fotos e documentos do fato', acao: 'wizard-step-4', icone: 'paperclip', tom: 'neutral', ativo: true, ordem: 8 },
  { id: 'historico', titulo: 'Etapa 6 • Histórico', subtitulo: 'Relato, providências e observações', acao: 'wizard-step-5', icone: 'history', tom: 'neutral', ativo: true, ordem: 9 },
  { id: 'revisao', titulo: 'Etapa 7 • Revisão', subtitulo: 'Validar e finalizar o boletim', acao: 'wizard-step-6', icone: 'check', tom: 'neutral', ativo: true, ordem: 10 },
  { id: 'banco', titulo: 'Banco Google Sheets', subtitulo: 'Configurar, testar e sincronizar', acao: 'show-bank', icone: 'database', tom: 'amber', ativo: true, ordem: 11 }
];
...
const state = {
  ...
  syncState: 'offline',
  user: null,
  menuItems: structuredClone(MENU_FALLBACK)
};
...
function loadUserSession() {
  try {
    ...
    if (!parsed || !parsed.nome || !parsed.registro || !parsed.turno) return null;
    return parsed;
  } catch {
    return null;
  }
...
function clearUserSession() {
  state.user = null;
  try { localStorage.removeItem(SESSION_KEY); } catch { memoryStorage.delete(SESSION_KEY); }
}

function menuToneClass(tom = 'neutral') {
  return tom === 'green' ? 'green' : tom === 'amber' ? 'amber' : tom === 'blue' ? 'blue' : 'neutral';
}

function menuIcon(iconName = 'clipboard') {
  return ICONS[iconName] || ICONS.clipboard;
}

function normalizeMenuItem(item = {}, index = 0) {
  return {
    id: String(item.id || item.ID || item.Id || `menu-${index + 1}`),
    titulo: String(item.titulo || item.Titulo || item.title || 'Menu').trim(),
    subtitulo: String(item.subtitulo || item.Subtitulo || item.subtitle || '').trim(),
    acao: String(item.acao || item.Acao || item.action || '').trim(),
    icone: String(item.icone || item.Icone || item.icon || 'clipboard').trim(),
    tom: String(item.tom || item.Tom || item.cor || item.Cor || 'neutral').trim().toLowerCase(),
    ativo: String(item.ativo ?? item.Ativo ?? 'true').toLowerCase() !== 'false',
    ordem: Number(item.ordem || item.Ordem || index + 1) || (index + 1)
  };
}

async function loadMenuData() {
  state.menuItems = structuredClone(MENU_FALLBACK);
  if (!apiConfigured()) return state.menuItems;
  try {
    const payload = await apiGet({ action: 'menu' });
    const items = Array.isArray(payload.items) ? payload.items.map(normalizeMenuItem).filter(item => item.ativo && item.titulo && item.acao) : [];
    if (items.length) {
      state.menuItems = items.sort((a, b) => a.ordem - b.ordem);
      state.syncState = 'online';
    }
  } catch (error) {
    console.warn('Menu não carregado da planilha:', error.message);
...
function createBlankRecord() {
  ...
  return {
    ...
    basic: {
      ...
      referencia: '',
      referenciaOutra: '',
      matriculaEmissor: state.user?.registro || '',
      nomeEmissor: state.user?.nome || '',
      emailEmissor: state.user?.email || '',
      local: '',
      complementoLocal: '',
      diretoria: '',
      diretoriaOutra: ''
...
function updateHeader() {
  const config = {
    login: ['Acesso do usuário', 'Identificação inicial'],
    menu: ['Menu operacional', state.user ? `${state.user.nome} • ${state.user.turno}` : 'Atalhos do aplicativo'],
  ...
  const hideChrome = ['login', 'menu'].includes(state.route);
  appHeader.classList.toggle('hidden', hideChrome);
  const showBack = ['wizard', 'detail'].includes(state.route);
  backButton.classList.toggle('hidden', !showBack);
  bottomNav.classList.toggle('hidden', hideChrome || state.route === 'wizard');
...
function renderLogin() {
    ...
    <section class="login-wrap">
      <div class="login-card">
        <div class="login-brand">
          <div class="stellantis-wordmark">STELLANTIS</div>
          <p class="eyebrow">Segurança patrimonial</p>
          <h1>Acesso ao aplicativo</h1>
          <p>Informe o usuário, o registro e o turno para iniciar o preenchimento dos boletins e acessar o menu operacional.</p>
        </div>
        <form id="login-form" class="login-form" novalidate>
          <div class="field full">
            <label class="required" for="login-nome">Usuário</label>
            <input id="login-nome" name="nome" type="text" placeholder="Nome do vigilante" value="${escapeHtml(state.user?.nome || '')}" required>
          </div>
          <div class="field full">
            <label class="required" for="login-registro">Registro</label>
            <input id="login-registro" name="registro" type="text" inputmode="numeric" placeholder="Ex.: 76313" value="${escapeHtml(state.user?.registro || '')}" required>
          </div>
          <div class="field full">
            <label class="required" for="login-turno">Turno</label>
            <select id="login-turno" name="turno" required>
              <option value="">Selecione o turno</option>
              ${['1º turno','2º turno','3º turno','Administrativo'].map(turno => `<option value="${turno}" ${state.user?.turno === turno ? 'selected' : ''}>${turno}</option>`).join('')}
            </select>
          </div>
          <button class="button primary giant" type="submit">${ICONS.shield} Entrar</button>
        </form>
      </div>
    </section>`;
  ...
  app.querySelector('#login-form').addEventListener('submit', async event => {
    ...
    const user = {
      ...
      registro: String(form.get('registro') || '').trim(),
      turno: String(form.get('turno') || '').trim()
    };
    if (!user.nome || !user.registro || !user.turno) {
      showToast('Preencha usuário, registro e turno para continuar.');
      event.currentTarget.querySelectorAll('input,select').forEach(el => {
        if (!el.value) el.classList.add('warning-field');
      });
      return;
    }
...
function renderMenu() {
  const items = (state.menuItems || []).filter(item => item.ativo !== false);
  app.innerHTML = `
    <section class="menu-wrap">
      <div class="menu-card-screen">
        <div class="menu-header-screen">
          <div class="stellantis-wordmark">STELLANTIS</div>
          <p class="eyebrow">Segurança patrimonial</p>
          <div class="menu-screen-topline">
            <div>
              <h1>Menu operacional</h1>
              <p>Selecione uma etapa do aplicativo para continuar.</p>
            </div>
            <button class="icon-button" type="button" data-menu-action="refresh-menu" aria-label="Atualizar menu">${ICONS.sync}</button>
          </div>
        </div>
        <div class="menu-button-list">
          ${items.map(item => `
            <button class="menu-screen-button" type="button" data-menu-go="${escapeHtml(item.acao)}" data-tone="${escapeHtml(menuToneClass(item.tom))}">
              <span class="menu-screen-icon">${menuIcon(item.icone)}</span>
              <span class="menu-screen-copy"><strong>${escapeHtml(item.titulo)}</strong>${item.subtitulo ? `<small>${escapeHtml(item.subtitulo)}</small>` : ''}</span>
              <span class="menu-screen-arrow">${ICONS.chevron}</span>
            </button>`).join('')}
        </div>
        <div class="menu-screen-footer">
          <div class="menu-user-chip">${ICONS.users}<span>${escapeHtml(state.user?.nome || 'Usuário')} / ${escapeHtml(state.user?.registro || '--')} • ${escapeHtml(state.user?.turno || '--')}</span></div>
          <div class="menu-footer-actions">
            <button class="button secondary" type="button" data-menu-action="switch-user">Trocar usuário</button>
            <button class="button warning" type="button" data-menu-action="finalizar-turno">Finalizar turno</button>
          </div>
        </div>
      </div>
    </section>`;

  app.querySelectorAll('[data-menu-go]').forEach(button => button.addEventListener('click', () => handleMenuAction(button.dataset.menuGo)));
  app.querySelector('[data-menu-action="switch-user"]').addEventListener('click', async () => {
    const confirmed = await openAppModal({ kind: 'warning', eyebrow: 'Trocar usuário', title: 'Deseja trocar o usuário atual?', message: 'O aplicativo voltará para a tela de login. Os boletins salvos neste dispositivo serão mantidos.', confirmText: 'Trocar usuário', cancelText: 'Cancelar' });
    if (!confirmed) return;
    clearUserSession();
    updateHeader();
    navigate('login');
  });
  app.querySelector('[data-menu-action="finalizar-turno"]').addEventListener('click', async () => {
    const confirmed = await openAppModal({ kind: 'warning', eyebrow: 'Encerramento de turno', title: 'Finalizar turno atual?', message: 'A sessão local será encerrada e o aplicativo retornará para a tela de login.', confirmText: 'Finalizar turno', cancelText: 'Cancelar' });
    if (!confirmed) return;
    clearUserSession();
    updateHeader();
    navigate('login');
  });
  app.querySelector('[data-menu-action="refresh-menu"]').addEventListener('click', async () => {
    await loadMenuData();
...
async function getDraftForMenu(requiredStep = 0) {
  const draft = state.records.find(record => record.status === 'Rascunho');
  if (draft) {
    state.current = structuredClone(draft);
    state.currentStep = requiredStep;
    return true;
  }
  const confirmed = await openAppModal({
    kind: 'info',
    eyebrow: 'Rascunho não encontrado',
    title: 'Criar um novo boletim?',
    message: 'Não existe rascunho em aberto neste dispositivo. Um novo boletim será criado para abrir a etapa selecionada.',
    confirmText: 'Criar novo boletim',
    cancelText: 'Cancelar'
  });
...
async function handleMenuAction(action) {
  if (action === 'new-bo') return createNewBo();
  if (action === 'show-records') return navigate('records');
  if (action === 'show-bank') return navigate('about');
  if (action === 'continue-draft') {
    const ok = await getDraftForMenu(0);
    if (ok) navigate('wizard');
    return;
  }
  if (action.startsWith('wizard-step-')) {
    const step = Number(action.replace('wizard-step-', ''));
    if (!Number.isInteger(step)) return;
    const ok = await getDraftForMenu(step);
    if (ok) navigate('wizard');
    return;
  }
  showToast('Ação do menu não reconhecida.');
}
...
function renderRecords() {
  ...
  app.innerHTML = `
    <section class="form-card">
      ...
      <div class="search-wrap">${ICONS.search}<input class="search-input" id="record-search" type="search" value="${escapeHtml(state.search)}" placeholder="Digite o termo da pesquisa"></div>
...
function renderAbout() {
  app.innerHTML = `
    ...
    <div class="about-list">
      ...
      <div class="code-note">Abas criadas: BO_Ocorrencias, BO_Pessoas, BO_Veiculos, BO_Materiais, BO_Anexos e BO_Menu.</div>
      <div class="notice" style="margin-top:14px"><strong>Menu do aplicativo:</strong> os botões da tela de menu são lidos da aba <strong>BO_Menu</strong> da planilha. Edite os títulos, subtítulos, ações, ícones e a ordem conforme necessário.</div>
      <div class="about-actions" style="margin-top:12px"><button class="button secondary" type="button" data-about-action="refresh-menu">${ICONS.sync} Atualizar menu da planilha</button></div>
    </div>
  ...
  app.querySelector('[data-about-action="save-settings"]').addEventListener('click', async () => {
    ...
    updateHeader();
    renderAbout();
    if (apiConfigured()) await loadMenuData();
    await openAppModal({
  ...
  const refreshBtn = app.querySelector('[data-about-action="refresh-menu"]');
  if (refreshBtn) refreshBtn.addEventListener('click', async () => { await loadMenuData(); await openAppModal({ kind: 'success', eyebrow: 'Menu operacional', title: 'Menu atualizado', message: `Foram carregados ${state.menuItems.length} item(ns) da aba BO_Menu.`, confirmText: 'Continuar' }); });
}
...
backButton.addEventListener('click', async () => {
  ...
  else if (state.route === 'detail') navigate('records');
  else navigate(state.user ? 'menu' : 'login');
});

bottomNav.addEventListener('click', event => {
  const button = event.target.closest('[data-route]');
  if (!button) return;
  navigate(button.dataset.route === 'home' ? (state.user ? 'menu' : 'login') : button.dataset.route);
});
...
async function init() {
  try {
    ...
    if (apiConfigured()) await loadMenuData();
    updateHeader();
    await navigate(state.user ? 'menu' : 'login');
    if ('serviceWorker' in navigator) navigator.serviceWorker.register('./service-worker.js?v=9', { updateViaCache: 'none' });
  } catch (error) {
    console.error(error);
